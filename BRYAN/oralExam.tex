...
\subsection{1989}
\subsubsection{Oakes, 1989 \cite{oakes1989bivariate}}
 Oakes studied association between two survival functions in the context of \emph{frailty} models $Pr(T>t|W=w) = \{B(t)\}^w$. It can be shown that for these models:
	$$
	\begin{aligned}
		S(t) &= Pr(T>t) = E_W[Pr(T>t,w)]=E_W e^{-(-logB(t))w} = p(-logB(t))
	\end{aligned}
	$$
 where $p(\cdot)$ is the Laplace transform of $W$ ($p(x) = Ee^{xW}$). This representation extends for bivariate distributions, $S(t_1, t_2) = Pr(T_1>t_1, T_2>t_2)$. In a bivariate frailty model $T_1$ and $T_2$ are conditionally independent, given W:
	$$
	\begin{aligned}
	...
	\end{aligned}
	$$
Oakes proves a lemma that Clayton's $\theta$ depends on $t=(t_1, t_2)$ only through $S(t)=v$ $\theta(v) = \theta^*(t) = \theta^*(S(t))$. He derives a general formula for $\theta(v)$ and also uses the relation $\frac{\theta(v)-1}{\theta(v)+1}$ to derive conditional version of \emph{Kendall}'s $\tau$ (or \emph{local} version of \emph{Kendall}'s $\tau$).
