...
\subsubsection{Fan, 2000 \cite{fan2000class}}
...
Which looks very much like:
	...
	\end{aligned}
	$$
Which is the coefficient of association derived by Clayton[\cite{clayton1978model}] for a specific model. In the context of  Clayton's model, this coefficient was a constant. Dabrowska's  $1-L(du, dv)$ is not a constant and is not a ratio of changes in survival functon, but the similarity is facionating.\\

Dabrowska addresses a comment of the reviewer that the suggested estimator has negative mass assignment. She provides an example of such case:
...
bivarSurvEstData = data.frame(X=X, Y=Y, lam11=lam11, lam10=lam10, lam01=lam01, KM1=rep(KM1, each=4), KM2 = rep(KM2, 4), L=L)
...
She suggests that this can be explained by the fact that $\hat{\Lambda}(\Delta t_1, \Delta t_2)$, $\lambda_{10}(s,t) = \lim_{h\rightarrow 0}$, and $\lambda_{01}(s,t) = \lim_{h\rightarrow 0}$ summarize three different portions of the data. She also notices that in the absence of censoring, this problem does not arise.
