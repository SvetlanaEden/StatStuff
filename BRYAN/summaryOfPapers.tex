...
When estimating the variance of PSR correlation, we use Kaplan-Meier as an estimate of survival function and treat it as a known \emph{parameter}. But, Kaplan-Meier also has variance and we need to take it into account in order get a good estimate for PSR correlation variance.\\
...
$$
  \begin{aligned}
    \psi_{Y}(\hat{F}_{1,Y}(y_i), y_i) &= V_{1,i,Y} - \hat{F}_{1,Y}(y_i)\\
    \psi_{Y}(\hat{F}_{2,Y}(y_i), y_i) &= V_{2,i,Y} - \hat{F}_{2,Y}(y_i)\\
    ...\\
    \psi_{Y}(\hat{F}_{k,Y}(y_i), y_i) &= V_{k,i,Y} - \hat{F}_{k,Y}(y_i)\\
  \end{aligned}
$$
Where:
...
$$
  \begin{aligned}
    ...
    \gamma_1(Y_i) &=  \frac{1}{1-H(Y_i)}    \int I_{\{ Y_i<w \}} \phi_j(w)  \gamma_0(w)H^1(dw)   \\
    \gamma_2(Y_i) &=   \int \int   \frac{I_{\{ v<Y_i,~v<w \}} \phi_j(w) \gamma_0(w)}{[1-H(v)]^2}   H^0(dv)   H^1(dw)   \\
    H^0(y) &= P\{  Y\leq y, ~\epsilon = 0 \} = \frac{(1-\epsilon_i)I_{\{ Y_i \leq y \}}}{N} \\
    H^1(y) &= P\{  Y\leq y, ~\epsilon = 1 \} = \frac{\epsilon_iI_{\{ Y_i \leq y \}}}{N} \\
    H(y) &= P\{  Y\leq y\} = \frac{I_{\{ Y_i \leq y \}}}{N} \\
  \end{aligned}
$$
...
% \ref{Defining:Matr}, the first delta method step is used to estimate the variance-covariance matrix for $\hat{\theta}_1$, $\hat{\theta}_2$, $\hat{\theta}_3$, $\hat{\theta}_4$, and $\hat{\theta}_5$. The second delta method step is used to estimate $\rho_S = \frac{\theta_3 - \theta_1 \theta_2}{\sqrt{\theta_4 - \theta_1^2}\sqrt{\theta_4 - \theta_2^2}}$. Let's proceed with the first delta method step. Let's further denote a Kaplan-Meier estimate of $F_X(x)$ in point $x_i$ as $\hat{F}_X(x_i)$. In the context of this kind of estimation, $\hat{F}_X(x_i^-) = \hat{F}_X(x_{i-1})$, so we can write the following: $\widehat{PSR}_{X_i} = \hat{F}_X(x_i) + \hat{F}_X(x_{i-1})\delta_i - \delta_i$. We remember that each $\hat{F}_X(x_i)$ is a random parameter estimate and proceed to write:
%
% {\footnotesize{
% $$
%   \begin{aligned}
%     g_1(\pmb{PSR}_{X_i}, \pmb{PSR}_{Y_i}) &= \frac{1}{N}\sum_{i=1}^N PSR_{X_i} = \frac{1}{N}\sum_{i=1}^N \left[  \hat{F}_X(x_i) + \hat{F}_X(x_{i-1})\delta_i - \delta_i   \right]\\
%     g_2(\pmb{PSR}_{Y_i}, \pmb{PSR}_{Y_i}) &= \frac{1}{N}\sum_{i=1}^N PSR_{Y_i} = \frac{1}{N}\sum_{i=1}^N \left[  \hat{F}_Y(y_i) + \hat{F}_Y(y_{i-1})\epsilon_i - \epsilon_i   \right]\\
%     g_3(\pmb{PSR}_{X_i}, \pmb{PSR}_{Y_i}) &= \frac{1}{N}\sum_{i=1}^N PSR_{X_i}PSR_{Y_i} = \frac{1}{N}\sum_{i=1}^N \left[  \hat{F}_X(x_i) + \hat{F}_X(x_{i-1})\delta_i - \delta_i   \right]  \cdot  \left[  \hat{F}_Y(y_i) + \hat{F}_Y(y_{i-1})\epsilon_i - \epsilon_i   \right]\\
%     g_4(\pmb{PSR}_{X_i}, \pmb{PSR}_{Y_i}) &= \frac{1}{N}\sum_{i=1}^N PSR_{X_i}^2 = \frac{1}{N}\sum_{i=1}^N \left[  \hat{F}_X(x_i) + \hat{F}_X(x_{i-1})\delta_i - \delta_i   \right]^2\\
%     g_5(\pmb{PSR}_{X_i}, \pmb{PSR}_{Y_i}) &= \frac{1}{N}\sum_{i=1}^N PSR_{Y_i}^2 = \frac{1}{N}\sum_{i=1}^N \left[  \hat{F}_Y(y_i) + \hat{F}_Y(y_{i-1})\epsilon_i - \epsilon_i   \right]^2\\
%   \end{aligned}
% $$
% }}
% Let's write the derivatives while switching to shorter notations $F_X(x_1) = F_{X,1}$ and $PSR_{.,i} = R_{.,i}$:
% {\footnotesize{
%   $$
%   \begin{aligned}
%     \left. \left(    \frac{\partial g_1(., .)}{\partial F_{X,1}}, ..., \frac{\partial g_1(., .)}{\partial F_{X,N}}, ~ \frac{\partial g_1(., .)}{\partial F_{Y,1}},  ..., \frac{\partial g_1(., .)}{\partial F_{Y,N}}   \right)    \right|_{\hat{F}_X, \hat{F}_Y} &= \frac{1}{N}\left(   \delta_1, ~(1+\delta_2), ~(1+\delta_3), ...  , ~(1+\delta_{N-1}),  ~1,  \underbrace{~~0,~0,~0,...,~0,~0}_{N~for~Y} \right)\\
%     %%%%%%
%     \left. \left(    \frac{\partial g_2(., .)}{\partial F_{X,1}}, ..., \frac{\partial g_2(., .)}{\partial F_{X,N}}, ~ \frac{\partial g_2(., .)}{\partial F_{Y,1}},  ..., \frac{\partial g_2(., .)}{\partial F_{Y,N}}    \right)    \right|_{\hat{F}_X, \hat{F}_Y} &= \frac{1}{N}\left( \underbrace{0,~0,~0,...,~0,~0}_{N~for~X},~~  \epsilon_1, ~(1+\epsilon_2), ~(1+\epsilon_3), ...  , ~(1+\epsilon_{N-1}),  ~1 \right)\\
%     %%%%%%
%     \left. \left(    \frac{\partial g_3(., .)}{\partial F_{X,1}}, ..., \frac{\partial g_3(., .)}{\partial F_{X,N}}, ~ \frac{\partial g_3(., .)}{\partial F_{Y,1}},  ..., \frac{\partial g_3(., .)}{\partial F_{Y,N}}   \right)    \right|_{\hat{F}_X, \hat{F}_Y} &= \frac{1}{N}\left( \underbrace{R_{Y,1} +  R_{Y,2}\delta_2,  ...  , ~R_{Y,N-1} +  R_{Y,N}\delta_N,  ~R_{Y,N}}_{for~X},~~ \underbrace{R_{X,1} +  R_{X,2}\epsilon_2,  ...  , ~R_{X,N}}_{similar for Y} \right)\\
%     %%%%%%
%     \left. \left(    \frac{\partial g_4(., .)}{\partial F_{X,1}}, ..., \frac{\partial g_4(., .)}{\partial F_{X,N}}, ~ \frac{\partial g_4(., .)}{\partial F_{Y,1}},  ..., \frac{\partial g_4(., .)}{\partial F_{Y,N}}   \right)    \right|_{\hat{F}_X, \hat{F}_Y} &= 2\frac{1}{N}\left( R_{X,1} +  R_{X,2}\delta_2, ~R_{X,2} +  R_{X,3}\delta_3,  ...  , ~R_{X,N-1} +  R_{X,N}\delta_N,  ~R_{X,N},  \underbrace{~~0,~0,~0,...,~0,~0}_{N~for~Y} \right)\\
%     %%%%%%
%     \left. \left(    \frac{\partial g_5(., .)}{\partial F_{X,1}}, ..., \frac{\partial g_5(., .)}{\partial F_{X,N}}, ~ \frac{\partial g_5(., .)}{\partial F_{Y,1}},  ..., \frac{\partial g_5(., .)}{\partial F_{Y,N}}   \right)    \right|_{\hat{F}_X, \hat{F}_Y} &= 2\frac{1}{N}\left( \underbrace{~~0,~0,~0,...,~0,~0}_{N~for~X},~~ R_{Y,1} +  R_{Y,2}\epsilon_2, ~R_{Y,2} +  R_{Y,3}\epsilon_3,  ...  , ~R_{Y,N-1} +  R_{Y,N}\epsilon_N,  ~R_{Y,N} \right)\\
%   \end{aligned}
%   $$
% }}
%
% We are still doing the first delta method step and now we are concerned with variance covariance matrix of $\hat{F_.(.)}$. According to \cite{breslow1974large}, it is defined as:
% $$
%   \begin{aligned}
%     Cov\left( F(s), F(t)  \right) &= (1 -F^o(s)) (1 -F^o(t)) \int_0^{s}  \frac{dF^o(x)}{ (1 -F(x^-))  (1 -F^o(x)) },
%     &\text{for }  s\leq t
%   \end{aligned}
% $$
% Where $F^o(s)$ is the true distribution function (when there is not censoring) and I am not sure what $F(.)$ is...\\
%
% I am just going to do the following substitution:
% $$
%   \begin{aligned}
%     \widehat{Cov}\left( F(s), F(t)  \right) &= (1 -\hat{F}^o(s)) (1 -\hat{F}^o(t)) \sum_{x_i \leq s } \frac{\delta_i(?)}{ (1 -\hat{F^o}(x_{i-1}))  (1 -\hat{F}^o(x_i)) },
%     &\text{for }  s\leq t
%   \end{aligned}
% $$
%
% The variance of survival curve using Greenwood formula (Survival Analysis by Klein and Moeschberger, p. 92) is: $\widehat{V}[\hat{S}(t)] = \hat{S}^2(t)\cdot \sum_{t_i\leq t} \frac{\delta_i}{Y_i(Y_i - \delta_i)}$, where $Y_i$ is number at risk. It looks sort of similar...\\
% So now, we only need to compute it...
...
\begin{enumerate}
  ...
  \item double space the paper and send it to Bryan
  
  \item look for estimating equations for KM in Stute's paper
  ...
  \item send the Summary...pdf
  \item add confidence intervals for PSR correlation
  \item Implemented M-estimation for Cox using the full likelihood, but it did not work
  \item tried to look at Qi's conditional correlation, but did not get it yet.
...
\begin{enumerate}
  ...
  \item check why the power goes down with the sample size
  \item previous 
