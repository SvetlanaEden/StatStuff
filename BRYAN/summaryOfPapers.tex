...
\textbf{Note} that in order to simmulate this distribution, it is not necessary to use copulas, we do this in order to practice the technique of using copulas. We can simulate the marginal distribution for $X$ first ($X \sim f_X(x) = e^{-x}$). Then we can use conditional distribution function for $Y$ in order to sample $Y$: $F_Y(y|X=x) = e^{-x} - (1+\theta y)e^{-(x+y+\theta xy)}$. We first simmulate $x_i \sim e^{-x}$ and $t_i \sim Unif(t)$ and then for each $x_i$ and $t_i$ we find $y_i$ by numerically inverting expression: $t_i = e^{-x_i} - (1+\theta y_i)e^{-(x_i+y_i+\theta x_iy_i)}$ \\
~\\
