...
We use the following notations. We pick a pair $(X_{0},Y_{0})$ and two independent variables $(X_{1}$ and $Y_{2})$. We denote the pdf and the survival functions as $f(x,y)$ and $S(x,y)$ respectively. Let's first find the following probability:
  
	$$
	\begin{aligned}
		P\{X_{0}\wedge X_{1} > x,~Y_{0}\wedge Y_{2} > y \} &= P\{ X_{0} > x, X_{1} > x, Y_{0} > y, Y_{2} > y \} = S(x, y)\cdot S_X(x) \cdot S_Y(y)\\
    &therefore\\
    f(X_{0}\wedge X_{1} > x,~Y_{0}\wedge Y_{2} > y)& = \frac{\partial^2 [S(x^-, y^-)\cdot S_X(x^-) \cdot S_Y(y^-)]}{\partial x \partial y}=\\
...
Next, we show that the \textbf{joint} distribution of concordance \textbf{and} of $X_{0}\wedge X_{1} = x,~Y_{0}\wedge Y_{2} = y$ is the following:
    ...
    &=P\{ X_{0} > x, X_{1} = x, Y_{0} > y, Y_{2} = y \} + P\{ X_{0}= x, X_{1} > x, Y_{0}= y, Y_{2} > y \} =\\
...
Similarly, we show that the \textbf{joint} distribution of discordance \textbf{and} of $X_{0}\wedge X_{1} = x,~Y_{0}\wedge Y_{2} = y$ is the following:
    ...
    &=P\{ X_0 > x, X_1 = x, Y_0 = y, Y_{2} > y \} + P\{ X_0 = x, X_1 > x, Y_0> y, Y_{2} = y \} = \\
...
Here we explain how \cite{oakes1989bivariate} showed that $\tau(s_1, s_2)$ can be written as:
	...
	\end{aligned}
	$$
For convenience let's switch from notations $(T_{11},T_{12})$ and $(T_{21},T_{12})$ to $(X_{1},Y_{1})$ and $(X_{2},Y_{2})$. We denote the pdf and the survival functions as $f(x,y)$ and $S(x,y)$ respectively. Let's first find the distribution (\emph{pdf}) for $X_{1}\wedge X_{2} = x,~Y_{1}\wedge Y_{2} = y$. We will do this using a trick. We'll first find $S(x, y)$ for $X_{1}\wedge X_{2} = x,~Y_{1}\wedge Y_{2} = y$ and then we will use the relationship: $f(x,y) = \frac{\partial^2 S(x^-, y^-)}{\partial x \partial y}$
  ...
  % $$
	...
	$$
	\begin{aligned}
		S_{min(X_1, X_2),min(Y_1, Y_2)}(x, y) &= P\{ X_{1}\wedge X_{2} > x,~Y_{1}\wedge Y_{2} > y \} = P\{ X_{1} > x, X_{2} > x, Y_{1}> y, Y_{2} > y \} =\\
    &= P\{ X_{1} > x, Y_{2} > y, X_{2} > x, Y_{1}> y\} = P\{ X_{1} > x, Y_{2} > y \}  \cdot P\{X_{2} > x, Y_{1}> y\}  =\\
    &= S^2(x, y)\\
	\end{aligned}
	$$
  Using the trick:
...
Now, let's find the joint distribution of concordance and of $X_{1}\wedge X_{2} = x,~Y_{1}\wedge Y_{2} = y$ by finding first the following:
	$$
	\begin{aligned}
		P\{ (X_1>X_2, Y_1>Y_2) \cap (X_{1}\wedge X_{2} = x,~Y_{1}\wedge Y_{2} = y) \} &=P\{ X_1>x, X_2=x, Y_1>y, Y_2=y)\} = \\
    &= P\{ X_1>x, Y_1>y, X_2=x, Y_2=y)\} = S(x, y)f(x,y)
	\end{aligned}
	$$
  Because of symmetry:
	$$
	\begin{aligned}
		P\{ (X_1<X_2, Y_1<Y_2) \cap (X_{1}\wedge X_{2} = x,~Y_{1}\wedge Y_{2} = y) \} &= S(x, y)f(x,y)
	\end{aligned}
	$$
  For discordance:
	$$
	\begin{aligned}
		P\{ (X_1>X_2, Y_1<Y_2) \cap (X_{1}\wedge X_{2} = x,~Y_{1}\wedge Y_{2} = y) \} &=P\{ X_1>x, X_2=x, Y_1=y, Y_2>y)\} = \\
    &= P\{ X_1>x, Y_1=y, X_2=x, Y_2>y)\} = \frac{\partial S(x, y^-)}{\partial y} \frac{\partial S(x^-, y)}{\partial x} 
	\end{aligned}
	$$
  Because of symmetry:
	$$
	\begin{aligned}
		P\{ (X_1<X_2, Y_1>Y_2) \cap (X_{1}\wedge X_{2} = x,~Y_{1}\wedge Y_{2} = y) \} &= \frac{\partial S(x, y^-)}{\partial y} \frac{\partial S(x^-, y)}{\partial x} 
	\end{aligned}
	$$
...
Therefore
  ...
  \begin{aligned}
    \tau(x, y) &= \frac{P\{concord| X_{1}\wedge X_{2} = x,~Y_{1}\wedge Y_{2} = y\} - P\{discord|X_{1}\wedge X_{2} = x,~Y_{1}\wedge Y_{2} = y\}}{P\{X_{1}\wedge X_{2} = x,~Y_{1}\wedge Y_{2} = y\}} = \\ 
     &= \frac{2S(x,y)S(dx,dy) - 2 S(x, dy)S(dx, y) }{ 2S(x^-,y^-)f(x, y) + 2\frac{\partial S(x^-,y)}{\partial y}\frac{\partial S(x,y^-)}{\partial x} } =\\
...
\psi_i(X_i, Y_i, \pmb{Z}_i; \pmb{\theta}) = \begin{bmatrix}
    ...
    (PSR_{Y_{i}}(\pmb{\theta}_Y))^2 -\theta_1 \\
...
\subsection{M-estimation in case of normal distribution}
Let's figure out $\pmb{A}_{Y,2,2}$, $\pmb{A}_{Y, 4,2}$, $\pmb{A}_{Y,5,2}$, $\pmb{A}_{Y, 7,2}$, for normally distributed $Y$'s (the corresponding matrices for $X$, $\pmb{A}_{X,1,1}$, $\pmb{A}_{X, 3,1}$, $\pmb{A}_{X,5,1}$, $\pmb{A}_{X, 6,1}$, can be found in a similar manner). For linear regression $Y\sim Z$, the score equations are obtained straight from the likelihood and for $\pmb{\theta} = (\beta_0, \beta_1, \beta_2)$, for example, they are:
	$$
	\begin{aligned}
		&y_i - \beta_0 - \beta_1z_{i1} - \beta_2z_{i2}=0\\
		&z_{1i}(y_i - \beta_0 - \beta_1z_{i1} - \beta_2z_{i2})=0\\
		&z_{2i}(y_i - \beta_0 - \beta_1z_{i1} - \beta_2z_{i2})=0\\
		&-\sigma^2 + (y_i - \beta_0 - \beta_1z_{i1} - \beta_2z_{i2})^2=0\\
 	\end{aligned}
	$$
Let's denote $res_i = y_i - \beta_0 - \beta_1z_{1i} - \beta_2z_{2i}$:  

\[
\pmb{A}_{Y,2,2} = \frac{1}{n}\sum_i^n\begin{bmatrix}
    1   &  z_{i1}   & z_{i2}  &  0\\
    z_{i1}   &  z_{i1}^2   & z_{i1}z_{i2}  &  0 \\
    z_{i2}   &  z_{i1}z_{i2}   & z_{i2}^2  &  0 \\
...
Where $\pmb{res}=\pmb{Y}_i - \pmb{Z}_i\pmb{\theta}_Y$. Now let's see what $\pmb{A}_{Y, 4,2}$ is. Because we are in the context of linear model, $Y \sim Normal(\pmb{Z} \cdot \pmb{\theta}_Y , \sigma^2)$. Then the $PSR_Y(\pmb{\theta}_Y)$
  $$
  \begin{aligned}
    PSR_Y(\pmb{\theta}_Y) = 2F_{\pmb{\theta}_Y}(\pmb{Y}) - 1 = 2 \Phi\left(\frac{\pmb{Y} - \pmb{Z}\pmb{\theta}_Y}{\sigma}\right) - 1
  \end{aligned}
  $$
Where $F_{\pmb{\theta}_Y}(\cdot)$ is a \emph{cdf} of $Normal(\pmb{Z} \cdot \pmb{\theta}_Y , \sigma^2)$ and $\Phi(\cdot)$ is a \emph{cdf} of a standard normal, $Normal(\pmb{0}, 1)$. In order to find $\pmb{A}_{Y, 4,2}$, we have to find  $\frac{\partial PSR_Y(\pmb{\theta}_Y)}{\partial \beta_0}$, $\frac{\partial PSR_Y(\pmb{\theta}_Y)}{\partial \beta_1}$, $\frac{\partial PSR_Y(\pmb{\theta}_Y)}{\partial \beta_2}$, and $ \frac{\partial PSR_Y(\pmb{\theta}_Y)}{\partial \sigma}$:

  $$
  \begin{aligned}
    \frac{\partial PSR_{Y_i}(\pmb{\theta}_Y)}{\partial \beta_0} &= -2f\left(\frac{Y_i - \pmb{Z}_i\pmb{\theta}_Y}{\sigma}\right)\frac{1}{\sigma}\\
    \frac{\partial PSR_{Y_i}(\pmb{\theta}_Y)}{\partial \beta_1} &= -2f\left(\frac{Y_i - \pmb{Z}_i\pmb{\theta}_Y}{\sigma}\right)\frac{z_{i,1}}{\sigma}\\
    \frac{\partial PSR_{Y_i}(\pmb{\theta}_Y)}{\partial \beta_2} &= -2f\left(\frac{Y_i - \pmb{Z}_i\pmb{\theta}_Y}{\sigma}\right)\frac{z_{i,2}}{\sigma}\\
    \frac{\partial PSR_{Y_i}(\pmb{\theta}_Y)}{\partial \sigma} &= +2f\left(\frac{Y_i - \pmb{Z}_i\pmb{\theta}_Y}{\sigma}\right)\frac{Y_i - \pmb{Z}_i\pmb{\theta}_Y}{\sigma^2}\\
  \end{aligned}
  $$
...
\pmb{A}_{Y,4,2} = \frac{1}{n}\sum_i^n\begin{bmatrix}
   2f\left(\frac{Y_i - \pmb{Z}_i\pmb{\theta}_Y}{\sigma}\right)\frac{1}{\sigma}   &  2f\left(\frac{Y_i - \pmb{Z}_i\pmb{\theta}_Y}{\sigma}\right)\frac{z_{i,1}}{\sigma}   & 2f\left(\frac{Y_i - \pmb{Z}_i\pmb{\theta}_Y}{\sigma}\right)\frac{z_{i,2}}{\sigma}  &  -2f\left(\frac{Y_i - \pmb{Z}_i\pmb{\theta}_Y}{\sigma}\right)\frac{Y_i - \pmb{Z}_i\pmb{\theta}_Y}{\sigma^2}\\
\end{bmatrix}\]

Or
\[
\pmb{A}_{Y,4,2} = \frac{2}{n\sigma}\begin{bmatrix}
   \left[f\left(\frac{\pmb{res}}{\sigma}\right)\right]^T \pmb{Z}, &
   -\left[f\left(\frac{\pmb{res}}{\sigma}\right)\right]^T \frac{\pmb{res}}{\sigma} \\
\end{bmatrix}
\]
...
\pmb{A}_{Y,5,2} = \frac{2}{n\sigma}\begin{bmatrix}
   \left[f\left(\frac{\pmb{res}}{\sigma}\right)\pmb{PSR}_X\right]^T \pmb{Z}, &
   -\left[f\left(\frac{\pmb{res}}{\sigma}\right)\pmb{PSR}_X\right]^T \frac{\pmb{res}}{\sigma} \\
\end{bmatrix}
\]
%%%%%%%%%%% A_{Y,7,2}
\[
\pmb{A}_{Y,7,2} = \frac{4}{n\sigma}\begin{bmatrix}
   \left[f\left(\frac{\pmb{res}}{\sigma}\right)\pmb{PSR}_Y\right]^T \pmb{Z}, &
   -\left[f\left(\frac{\pmb{res}}{\sigma}\right)\pmb{PSR}_Y\right]^T \frac{\pmb{res}}{\sigma} \\
\end{bmatrix}
\]

\subsubsection{M-estimation in case of normal distribution, Qi's approach}
