...
Here how we are planning to proceed. \cite{breslow1974large} established limitting normality for the product limit (Kaplan-Meier) estimates. Because PSR is a function of product limit estimates, we can use delta method to derive variance estimate for this function. For convenience (because I don't want to be overwhelmed by a bunch of derivatives), I will use delta method twice. Using notations of section \ref{Defining:Matr}, the first delta method step is used to estimate the variance-covariance matrix of $\hat{\theta}_1$, $\hat{\theta}_2$, $\hat{\theta}_3$, $\hat{\theta}_4$, and $\hat{\theta}_5$. The second delta method step is used to estimate $\rho_S = \frac{\theta_3 - \theta_1 \theta_2}{\sqrt{\theta_4 - \theta_1^2}\sqrt{\theta_4 - \theta_2^2}}$. Let's proceed with the first delta method step. Let's further denote a Kaplan-Meier estimate of $F_X(x)$ in point $x_i$ as $\hat{F}_X(x_i)$. In the context of this kind of estimation, $\hat{F}_X(x_i^-) = \hat{F}_X(x_{i-1})$, so we can write the following: $\widehat{PSR}_{X_i} = \hat{F}_X(x_i) + \hat{F}_X(x_{i-1})\delta_i - \delta_i$. We remember that each $\hat{F}_X(x_i)$ is a random parameter estimate and proceed to write:
...
Let's write the derivatives while switching to shorter notations $F_X(x_1) = F_{X,1}$:
{\footnotesize{  
  $$
  \begin{aligned}
    ...
    %%%%%%
    \left. \left(    \frac{\partial g_1(., .)}{\partial F_{X,1}}, ..., \frac{\partial g_1(., .)}{\partial F_{X,N}}, ~ \frac{\partial g_1(., .)}{\partial F_{Y,1}},  ..., \frac{\partial g_1(., .)}{\partial F_{Y,N}}   \right)    \right|_{\hat{F}_X, \hat{F}_Y} &= \sum_{i=1}^N \left[  \hat{F}_X(x_i) + \hat{F}_X(x_{i-1})\delta_i - \delta_i   \right]  \cdot  \left[  \hat{F}_Y(y_i) + \hat{F}_Y(y_{i-1})\epsilon_i - \epsilon_i   \right]\\
    %%%%%%
    \left. \left(    \frac{\partial g_1(., .)}{\partial F_{X,1}}, ..., \frac{\partial g_1(., .)}{\partial F_{X,N}}, ~ \frac{\partial g_1(., .)}{\partial F_{Y,1}},  ..., \frac{\partial g_1(., .)}{\partial F_{Y,N}}   \right)    \right|_{\hat{F}_X, \hat{F}_Y} &= \sum_{i=1}^N \left[  \hat{F}_X(x_i) + \hat{F}_X(x_{i-1})\delta_i - \delta_i   \right]^2\\
    %%%%%%
    \left. \left(    \frac{\partial g_1(., .)}{\partial F_{X,1}}, ..., \frac{\partial g_1(., .)}{\partial F_{X,N}}, ~ \frac{\partial g_1(., .)}{\partial F_{Y,1}},  ..., \frac{\partial g_1(., .)}{\partial F_{Y,N}}   \right)    \right|_{\hat{F}_X, \hat{F}_Y} &= \sum_{i=1}^N \left[  \hat{F}_Y(y_i) + \hat{F}_Y(y_{i-1})\epsilon_i - \epsilon_i   \right]^2\\
  \end{aligned}
  $$
}}
...
\begin{enumerate}
  ...
  \item I tried partial PSR simulation with a model that uses splines. They return a non-invertable variance matrix. It is a problem because the inverse of the variace matrix is necessary for M-estimation. I've rewritten the M-estimation function in order to accomodate for the fact that in survival regression, we can supply the inverse of sub matrix A.
