...
But we are going to use different parameterization because this is what \texttt{survreg()} function is using:
  ...
  \end{aligned}
  $$
So the residuals and their derivatives by $\beta_j$ can be written as:
  $$
  \begin{aligned}
...
% \textbf{Logistic} distribution:\\
  $$
  \begin{aligned}
    f(x) &= \frac{1}{\sqrt{2 \pi }\sigma x} e^{-\frac{(log(x) - \pmb{z}_i\pmb{\beta})^2}{2\sigma^2}}\\
    F(x) &= \frac{1}{2} + \frac{1}{\sqrt{\pi}} \int_0^x  e^{-\frac{(log(t) - \pmb{z}_i\pmb{\beta})^2}{2\sigma^2}} dt\\
  \end{aligned}
  $$
