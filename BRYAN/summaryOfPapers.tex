...
When we censor data at points $u_0$ and $v_0$, we know that the mean of PSR is still 0, and in order to find the variance we need to find:
	$$
	\begin{aligned}
	...
	\end{aligned}
	$$
We will use this result in order to compute correlation of PSR in the following two sections.
...
\pmb{A}_{SM, LM} = -
	\begin{bmatrix}
		\sum_i\frac{\partial \psi_{i,SM_{Y}}}{\partial \pmb{\alpha}_Y} & 0  & 0  & 0  & 0 \\
    %%%%%%%%%%%%
		0 & \sum_i\frac{\partial \psi_{i,SM_{X}}}{\partial \pmb{\alpha}_X} & 0  & 0  & 0  \\
    %%%%%%%%%%%%
		\sum_i\frac{\partial \psi_{i,LM_{r^2_Y}}}{\partial \pmb{\alpha}_Y} & 0  & \sum_i\frac{\partial \psi_{i,LM_{r^2_Y}}}{\partial \pmb{\beta}_Y}  & 0  & 0 \\
    %%%%%%%%%%%%
		0 & \sum_i\frac{\partial \psi_{i,LM_{r^2_X}}}{\partial \pmb{\alpha}_X} & 0  & \sum_i\frac{\partial \psi_{i,LM_{r^2_X}}}{\partial \pmb{\beta}_X}  & 0 \\
    %%%%%%%%%%%%
		\sum_i\frac{\partial \psi_{i,LM_{r_{YX}}}}{\partial \pmb{\alpha}_Y} & \sum_i\frac{\partial \psi_{i,LM_{r_{YX}}}}{\partial \pmb{\alpha}_X}  & 0  & 0  & \sum_i\frac{\partial \psi_{i,LM_{r_{YX}}}}{\partial \pmb{\beta}_{YX}} \\
 	\end{bmatrix} 
\]
...
Where $res_i = r^2_{Y_i}(\pmb{\alpha}_Y) - \pmb{\beta_{Y}}\pmb{t}_i$
{\small
{\[
\frac{\partial \pmb{\psi}_{i,LM_{r^2_Y}}}{\partial \pmb{\alpha}_Y} = 
	\begin{bmatrix}
		...
		4(r^2_{Y_i}(\pmb{\alpha}_Y) - \pmb{\beta_{Y}}\pmb{t}_i)\cdot r_{Y_i}(\pmb{\alpha}_Y) \frac{\partial r_{Y_i}(\pmb{\alpha}_Y)}{\partial \pmb{\alpha}_Y}
 	\end{bmatrix}=
% \]
% \[
2\cdot r_{Y_i}(\pmb{\alpha}_Y) 
	\begin{bmatrix}
		...
		t_{1i} & t_{1i} \\
		t_{2i} & t_{2i} \\
		2(r^2_{Y_i}(\pmb{\alpha}_Y) - \pmb{\beta_{Y}}\pmb{t}_i) & 2(r^2_{Y_i}(\pmb{\alpha}_Y) - \pmb{\beta_{Y}}\pmb{t}_i)\\
 	\end{bmatrix} 
  \circ
	\begin{bmatrix}
...
Similar formula holds for $\pmb{\psi}_{LM_{r^2_X}}$. Let's see what is going on for linear regression of $LM_{YX}$. We can use the type of score equations as $\pmb{\psi}_{LM_{r^2_Y}}$, but now $res_i = r_{Y_i}(\pmb{\alpha}_Y)r_{X_i}(\pmb{\alpha}_X) - \pmb{\beta}_{YX}\pmb{t}_i$, so
...
\frac{\partial \pmb{\psi}_{i,LM_{r_{YX}}}}{\partial \pmb{\alpha}_Y} =  r_{X_i}(\pmb{\alpha}_X)
  ...
  \circ
	\begin{bmatrix}
 	...
 	\end{bmatrix} 
\]
And $\frac{\partial \pmb{\psi}_{LM_{r_{YX}}}}{\partial \pmb{\alpha}_X}$ is computed in a similar manner:

Because $\sum_i\frac{\partial \psi_{i,LM_{r^2_Y}}}{\partial \pmb{\alpha}_Y}$ and $\sum_i\frac{\partial \psi_{i,LM_{r^2_X}}}{\partial \pmb{\alpha}_X}$ are defined in a similar manner, we only write down $\sum_i\frac{\partial \psi_{i,LM_{r^2_Y}}}{\partial \pmb{\alpha}_Y}$: 
...
\sum_i\frac{\partial \psi_{i,LM_{r^2_Y}}}{\partial \pmb{\alpha}_Y} = 
	\begin{bmatrix}
		...
		4\sum_i\frac{\partial r_{Y_i}(\pmb{\alpha}_Y)}{\partial \alpha_{Y,0}}r_{Y_i}(\pmb{\alpha}_Y)(r^2_{Y_i}(\pmb{\alpha}_Y) - \pmb{\beta_{Y}}\pmb{t}_i) & 4\sum_i\frac{\partial r_{Y_i}(\pmb{\alpha}_Y)}{\partial \alpha_{Y,1}}r_{Y_i}(\pmb{\alpha}_Y)(r^2_{Y_i}(\pmb{\alpha}_Y) - \pmb{\beta_{Y}}\pmb{t}_i)\\
 	\end{bmatrix} 
\]
}}

Because $\sum_i\frac{\partial \psi_{i,LM_{r_{YX}}}}{\partial \pmb{\alpha}_Y}$ and  $\sum_i\frac{\partial \psi_{i,LM_{r_{YX}}}}{\partial \pmb{\alpha}_X}$ are defined similarly, we only present $\sum_i\frac{\partial \psi_{i,LM_{r_{YX}}}}{\partial \pmb{\alpha}_Y}$:
...
As a result, we get a very large variance-covariance matrix, $\Sigma_{(\pmb{\alpha}_X, \pmb{\alpha}_X, \pmb{\beta}_{X^2}, \pmb{\beta}_{Y^2}, \pmb{\beta}_{XY})}$. We need only the part with $\beta$s, let's denote it as $\Sigma_{(\pmb{\beta}_{X^2}, \pmb{\beta}_{Y^2}, \pmb{\beta}_{XY})}$. Now, given this variance-covariance matrix, the values of $T$, and the $\Delta$-method, we can compute the variance of the expression: $\hat{\rho}_C(t) = \frac{  \widehat{r_{X_i}r_{Y_i}}(\pmb{\alpha}_Y, \pmb{\alpha}_X, ~\pmb{\beta}_{YX}, ~t)}{ \sqrt{ \widehat{r_{X_i}^2} (\pmb{\alpha}_X, ~\pmb{\beta}_{X^2},~\pmb{t}) \widehat{r_{Y_i}^2} (\pmb{\alpha}_Y, ~\pmb{\beta}_{Y^2},~\pmb{t})}}$. Note that although we have this expression depends on $\pmb{\alpha}$s, we don't have to worry about this because our $\Sigma_{(\pmb{\beta}_{X^2}, \pmb{\beta}_{Y^2}, \pmb{\beta}_{XY})}$ has taken this into account. So the resulting variance can be calculated like this:

$$
\begin{aligned}
  \left( \frac{\partial \hat{\rho}_C(t)}{\partial \pmb{\beta}_{X^2}}, \frac{\partial \hat{\rho}_C(t)}{\partial \pmb{\beta}_{Y^2}}, \frac{\partial \hat{\rho}_C(t)}{\partial \pmb{\beta}_{YX}}  \right)_{N\times 9}   \times~  \Sigma_{(\pmb{\beta}_{X^2}, \pmb{\beta}_{Y^2}, \pmb{\beta}_{XY}),~~9\times 9}  ~ \times  ~  \left( \frac{\partial \hat{\rho}_C(t)}{\partial \pmb{\beta}_{X^2}}, \frac{\partial \hat{\rho}_C(t)}{\partial \pmb{\beta}_{Y^2}}, \frac{\partial \hat{\rho}_C(t)}{\partial \pmb{\beta}_{YX}}  \right)^T_{9 \times N}
  \end{aligned}
$$

where $N$ is the number of points $t$ for which we are computing the conditional correlation and $9$ is the overall number of $\beta$ coefficients. Note that even if $t$ is scalar, the number of coefficients can be more than $2$, for example, when we model $t$ with restricted cubic splines with $3$ knots, the overall number of coefficients for each model including the intercept is $3$. Finally, let's write down the $i^{th}$ row of matrix $\left( \frac{\partial \hat{\rho}_C(t)}{\partial \pmb{\beta}_{X^2}}, \frac{\partial \hat{\rho}_C(t)}{\partial \pmb{\beta}_{Y^2}}, \frac{\partial \hat{\rho}_C(t)}{\partial \pmb{\beta}_{YX}}  \right)_{N\times 9}$ for clarity:

{\tiny
{$$
\begin{aligned}
  \left(\frac{-(1/2) \pmb{\beta}'_{YX}\pmb{t}_i} {\sqrt{(\pmb{\beta}'_{X^2}\pmb{t}_i)^3(\pmb{\beta}'_{Y^2}\pmb{t}_i)}}, \frac{-(1/2)(\pmb{\beta}'_{YX}\pmb{t}_i) t_{1,i}} {\sqrt{(\pmb{\beta}'_{X^2}\pmb{t}_i)^3(\pmb{\beta}'_{Y^2}\pmb{t}_i)}}, \frac{-(1/2)(\pmb{\beta}'_{YX}\pmb{t}_i) t_{2,i}} {\sqrt{(\pmb{\beta}'_{X^2}\pmb{t}_i)^3(\pmb{\beta}'_{Y^2}\pmb{t}_i)}}, 
  %%%%%%%
  \frac{-(1/2) \pmb{\beta}'_{YX}\pmb{t}_i} {\sqrt{(\pmb{\beta}'_{X^2}\pmb{t}_i)(\pmb{\beta}'_{Y^2}\pmb{t}_i)^3}}, \frac{-(1/2)(\pmb{\beta}'_{YX}\pmb{t}_i) t_{1,i}} {\sqrt{(\pmb{\beta}'_{X^2}\pmb{t}_i)(\pmb{\beta}'_{Y^2}\pmb{t}_i)^3}}, \frac{-(1/2)(\pmb{\beta}'_{YX}\pmb{t}_i) t_{2,i}} {\sqrt{(\pmb{\beta}'_{X^2}\pmb{t}_i)(\pmb{\beta}'_{Y^2}\pmb{t}_i)^3}},
  %%%%%%%
  \frac{1}{\sqrt{(\pmb{\beta}'_{X^2}\pmb{t}_i)(\pmb{\beta}'_{Y^2}\pmb{t}_i)}}, \frac{t_{1,i}}{\sqrt{(\pmb{\beta}'_{X^2}\pmb{t}_i)(\pmb{\beta}'_{Y^2}\pmb{t}_i)}}, \frac{t_{2,i}}{\sqrt{(\pmb{\beta}'_{X^2}\pmb{t}_i)(\pmb{\beta}'_{Y^2}\pmb{t}_i)}}  \right)_{N\times 9}  
  \end{aligned}
$$
}}
...
\begin{enumerate}
  ...
  \item Change label for Qi's paper because it is published now.
  \item Check definition of $\rho_S$ for discrete data ???
  %\item baseline hazard score equations: tute The central limit theorem under random censorship. Bryan's number: 615-624-2220
