...
\pmb{A}_{Y,2,2} = \frac{1}{n}\sum_i^n\begin{bmatrix}
    \left(\frac{\partial \pmb{\psi}_{Y_i}}{ \partial \beta_0} \right)^T \\
    \left(\frac{\partial \pmb{\psi}_{Y_i}}{ \partial \beta_1} \right)^T \\
    \left(\frac{\partial \pmb{\psi}_{Y_i}}{ \partial \beta_2} \right)^T \\
    \left(\frac{\partial \pmb{\psi}_{Y_i}}{ \partial (\sigma^2)} \right)^T \\
\end{bmatrix}
...
Now let's see what $\pmb{A}_{Y, 4,2}$ is. Because we are in the context of linear model, $Y \sim Normal(\pmb{Z} \cdot \pmb{\theta}_Y , \sigma^2)$. Then the $PSR_Y(\pmb{\theta}_Y)$
  $$
  \begin{aligned}
