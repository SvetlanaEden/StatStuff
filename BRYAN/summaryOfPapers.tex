...
We recall again the definition of PSR, $PSR_{Y_i} =  (1+\delta_i) F(y_i)  -  \delta_i = 1- (1+\delta_i) S(y_i)$. 
As usual, we only need to get $\frac{\partial PSR}{\partial \beta_i}$ and $\frac{\partial PSR}{\partial \lambda_0(y_i)}$, where $\lambda_0(y_i)$ is a parameter of the baseline hazard:
$$
  \begin{aligned}
    S(y_i) &=  e^{-\Lambda(y_i) e^{\pmb{z}_i\pmb{\beta}}} \\
    \Lambda(y_i)  &= \sum_{j\leq i} \hat{\lambda}_0(y_i)\\
    ...
    \frac{\partial PSR_{Y_i}}{\partial \lambda_0(y_i)} &=  (1+\delta_i)\cdot  e^{\pmb{z}_i\pmb{\beta}}   \cdot  e^{-\Lambda(y_i) e^{\pmb{z}_i\pmb{\beta}}}
  \end{aligned}
  $$
...
$$
  \begin{aligned}
    ...
    \frac{\partial PSR_{Y_i}}{\partial \lambda_0(y_i)} &=  (1+\delta_i)\cdot  e^{\pmb{z}_i\pmb{\beta}}   \cdot  S(y_i)
  \end{aligned}
  $$
\texttt{coxph} provides us with martingale residuals, so we would like to use them to compute $\frac{\partial PSR_{Y_i}}{\partial \beta_j}$ : 
  $$
  \begin{aligned}
    ...
    \frac{\partial PSR_{Y_i}}{\partial \lambda_0(y_i)} &=  (1+\delta_i)\cdot  e^{\pmb{z}_i\pmb{\beta}}   \cdot  e^{\hat{m}_i - \delta_i}
  \end{aligned}
  $$
  
This would have been the end of the story, but Cox regression is special because it uses partial likelihood, and therefore \texttt{R} provides us only with score equations for $\beta_j$. We, however, need score equations also for $\lambda_0(y_i)$. So, because baseline hazard is estimated in the following way: 
  $$
  \begin{aligned}
    \hat{\lambda}_0(y_i) &= \frac{\delta_i}{\sum_{k\in \mathcal{R}(y_i)} e^{\pmb{z_k}\pmb{\beta}}}~,~~~~
    where~\mathcal{R}(y_i)~is~a~risk~set
  \end{aligned}
  $$
  
Our score equation and the corresponding derivatives are:
  $$
  \begin{aligned}
    \psi_{{\lambda}_0(y_i)} &= \frac{\delta_i}{\sum_{k\in \mathcal{R}(y_i)} e^{\pmb{z_k}\pmb{\beta}}} - {\lambda}_0(y_i)\\
    \frac{\partial \psi_{\lambda_0(y_i)}}{  \partial \lambda_0(y_i)} &= - 1\\
    \frac{\partial \psi_{\lambda_0(y_i)}}{  \partial \beta_j} &= -\frac{\delta_i}{\left(   \sum_{k\in \mathcal{R}(y_i)} e^{\pmb{z_k}\pmb{\beta}}  \right)^2} \cdot \frac{ \partial \left(   \sum_{k\in \mathcal{R}(y_i)} e^{\pmb{z_k}\pmb{\beta}}  \right)}{\partial  \beta_j} = -\delta_i\frac{     \sum_{k\in \mathcal{R}(y_i)}  z_{kj}  e^{\pmb{z_k}\pmb{\beta}} }{\left(   \sum_{k\in \mathcal{R}(y_i)} e^{\pmb{z_k}\pmb{\beta}}  \right)^2} \\
  \end{aligned}
  $$
In order to have a general picture in our heads, let's outlike what the resulting $\pmb{A}_{Y,2,2}$ matrix looks like if we only had one $\beta_0$ coefficient and two subjects with unique failure points, $y_i$ and $y_2$. We keep in mind that $ \frac{\partial \pmb{\psi}_{Yi,\beta_0}}{ \partial \lambda_0(y_k)} = 0$ because $\pmb{\psi}_{Yi,\beta_0}$ is a result of partial likelihood that does not contain $\lambda_0(y_k)$:

  \[
  \pmb{A}_{Y,2,2} = -\sum_i^n\begin{bmatrix}
      \frac{\partial \pmb{\psi}_{Yi,\beta_0}}{ \partial \beta_0} &
      \frac{\partial \pmb{\psi}_{Yi,\beta_0}}{ \partial \lambda_0(y_1)} &
      \frac{\partial \pmb{\psi}_{Yi,\beta_0}}{ \partial \lambda_0(y_2)} \\
      %%%%%%%%%
      \frac{\partial \pmb{\psi}_{Yi,\lambda_0(y_1)}}{ \partial \beta_0} &
      \frac{\partial \pmb{\psi}_{Yi,\lambda_0(y_1)}}{ \partial \lambda_0(y_1)} &
      \frac{\partial \pmb{\psi}_{Yi,\lambda_0(y_1)}}{ \partial \lambda_0(y_2)} \\
      %%%%%%%%%
      \frac{\partial \pmb{\psi}_{Yi,\lambda_0(y_2)}}{ \partial \beta_0} &
      \frac{\partial \pmb{\psi}_{Yi,\lambda_0(y_2)}}{ \partial \lambda_0(y_1)} &
      \frac{\partial \pmb{\psi}_{Yi,\lambda_0(y_2)}}{ \partial \lambda_0(y_2)}\\
  \end{bmatrix}
   =-\sum_i^n\begin{bmatrix}
      \frac{\partial \pmb{\psi}_{Yi,\beta_0}}{ \partial \beta_0} & 0 & 0 \\
      %%%%%%%%%
      \frac{\partial \pmb{\psi}_{Yi,\lambda_0(y_1)}}{ \partial \beta_0} & -1 & 0 \\
      %%%%%%%%%
      \frac{\partial \pmb{\psi}_{Yi,\lambda_0(y_2)}}{ \partial \beta_0} & 0 & -1 \\
  \end{bmatrix}
  \]

Where $\frac{\partial \pmb{\psi}_{Yi,\lambda_0(y_1)}}{ \partial \beta_0}$ and $\frac{\partial \pmb{\psi}_{Yi,\lambda_0(y_2)}}{ \partial \beta_0}$ are derived above and $\frac{\partial \pmb{\psi}_{Yi,\beta_0}}{ \partial \beta_0}$ is provided by \texttt{coxph}.\\

From the above, we anticipate several challanges. One challange is the fact that we need to compute $\sum_{k\in \mathcal{R}(y_i)} e^{\pmb{z_k}\pmb{\beta}} $ manually (or using \texttt{R} cumulative hazard, it isn't clear yet). The second challenge is to invert matrix $\pmb{A}$. This is a challenge because if we have, for example, $n$ bivariate times to event with each time being a unique point and suppose each model has $p$ $\beta$ coefficients, the dimention of matrix $\pmb{A}$ will be $2p + 2n + 6$. We will need to invert it in a way that takes advantage of the fact that its largest parts are diagonal matrices.
