...
Bivariate surival function of the \emph{Clayton}'s family looks like this:
	...
	\end{aligned}
	$$
We chose the marginals to be $S_1(t_1) = e^{-\lambda_1t_1}$ and $S_2(t_2) = e^{-\lambda_2t_2}$. We simulate values of $T_2 = t^*_2$ and values of uniform variable $U = u$, and invert conditional survival function of $T_1$ in order to get $t_1$:
	$$
	\begin{aligned}
		 S(t_1|t^*_2) = u &= \left\{  S_1^{1-\gamma}(t_1) + S_2^{1-\gamma}(t^*_2) - 1 \right\}^{\frac{1}{1-\gamma}}\\
		 u^{1-\gamma} &+ 1 -S_2^{1-\gamma}(t^*_2) =   S_1^{1-\gamma}(t_1)\\
		 S_1(t_1) &=e^{-\lambda_1t_1}=  \left\{ u^{1-\gamma} + 1 -S_2^{1-\gamma}(t^*_2) \right\}^{\frac{1}{1-\gamma}} \\
		 -\lambda_1 t_1&= \frac{1}{1-\gamma}  ln\left\{ u^{1-\gamma} + 1 -S_2^{1-\gamma}(t^*_2) \right\} \\
		  t_1&= -\frac{1}{(1-\gamma)\lambda_1}  ln\left\{ u^{1-\gamma} + 1 - e^{-\lambda_2 t_2^*(1-\gamma)} \right\} \\
	\end{aligned}
	$$

Note that expression $u^{1-\gamma} + 1 - e^{-\lambda_2 t_2^*(1-\gamma)}$ can be $\leq 0$. If this is the case, we resample $t_2$ and $u$. Also, we continue resampling the values of $t_2$ and $u$ until the values of $t_1$ are all positive.
...
Under construction ...
\section{What I have been doing before this meeing}
\begin{enumerate}
  \item Implemented M-estimation for Cox using the full likelihood, but it did not work
  \item Tried to do Savage scores, but still working on them
  \item am trying to do Shih and Louis (martingale residuals paper), but don't understand it yet
  \item look at the simulations in two papers: Shih and Louis and Some other one about Normal Copula. It looks like I will have to implement the bivariate survival distributions mentioned in these papers
...
\section{Questions to Bryan}
\begin{enumerate}
  \item Ask Bryan, what families of bivariate survial to implement
  \item Ask what test characteristics to report in addition to type one error and power for different sample sizes and different cencoring proportions.
\end{enumerate}
