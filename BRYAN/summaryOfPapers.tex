...
%\linespread{1.3} for 1.5 space between lines
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%% for references I used to use biber
% %% when using biber as a "backend" compile like this:
% pdflatex myFile
% biber myFile
% pdflatex myFile
%%% But it stopped working, so now I am using latex command:
%     bibtex summaryOfPapers
...
% Therefore:.... the integral is complicated and I decided to use \emph{Spearman}'s $\rho$ anyway.
%   $$
%   \begin{aligned}
...
%%%%% the derivations below are incorrect because the Ec is not the same as Ex.
...
{\scriptsize{
	$$
	\begin{aligned}
    cov(U_0, V_0) &=  \underbrace{  \int_0^{V_0}\int_0^{U_0} (2U - 1)(2V - 1)\cdot H(dU, dV)      }_{\text{A}}  +   \underbrace{  \int_{V_0}^{1}\int_0^{U_0} (2U - 1)V_0\cdot H(dU, dV)  }_{B}   +
     \underbrace{\int_{0}^{V_0}\int_{U_0}^1 (2V - 1)U_0\cdot H(dU, dV)}_{C} +  \underbrace{\int_{V_0}^{1}\int_{U_0}^1 V_0 U_0\cdot H(dU, dV)}_{D}
	\end{aligned}
	$$
...
{\scriptsize{
	...
	\begin{aligned}
    cov(U_0, V_0) &= A+B+C+D = \\
    &= V_0 U_0 \large\{1 - U_0 - V_0  + H(U_0, V_0)\large\}+\\
    %%%%%%% integral C
    &+ U_0  \left\{  (2V_0 - 1)(V_0 - H(U_0, V_0)) - V_0^2 + 2 \int_0^{V_0} H(U_0, V)dV   \right\}+\\
    %%%%%%% integral B
    ...
    &+ (2U_0 - 1)(2V_0 - 1)H(U_0, V_0) - 2(2U_0 - 1)\int_0^{V_0} H(U_0, V)dV  - 2(2V_0 - 1)\int_0^{U_0} H(U, V_0)dU  + 4 \int_0^{U_0}\int_0^{V_0} H(U, V) dV  dU =\\
    %%%%%%% next step
    &= H(U_0, V_0)(1 - U_0)(1 - V_0) - U_0 V_0 + 2(1 - U_0)\int_0^{V_0} H(U_0, V)dV  + 2(1 - V_0)\int_0^{U_0} H(U, V_0)dU  + 4 \int_0^{U_0}\int_0^{V_0} H(U, V) dV  dU 
	\end{aligned}
	$$
...
{\scriptsize{
	$$
	\begin{aligned}
    var_X(U_0) = \frac{1}{3} U_0^3 - U_0^2 + U_0
	\end{aligned}
	$$
}}

In the following we use $G_{U_0}(du_0), G_{V_0}(dv_0)$ as a notation for marginal distribution of censoring variables on a cdf scale. We'll discuss how we get them later.\\
Question for Bryan and Chun: In order to compute the correlation of PSRs under censoring, $\rho^*_{PSRs}$ we use conditional covariance and variances and integrate over the censoring distribution. The question is how to integrate over the censoring distribution:

{\scriptsize{
	$$
	\begin{aligned}
   \text{This way: } \rho^*_{PSRs}  = \frac{\int \int cov(U_0, V_0) G_{U_0}(du_0)G_{V_0}(dv_0)}{\sqrt{\left(\int var_X(U_0)G_{U_0}(du_0) \right) \left(\int var_Y(V_0)G_{V_0}(dv_0)  \right)}}\\
    \text{or this way: } \rho^*_{PSRs}  = \int \int \frac{cov(U_0, V_0) }{ \sqrt{var_X(U_0)   var_Y(V_0)}}G_{U_0}(du_0)G_{V_0}(dv_0) \text{    ?}
	\end{aligned}
	$$
}}

Also, because the above formlas are on the cdf scale, there is another question, what are $G_{U_0}$ and $G_{V_0}$ in case when each outcome is distributed exponentially with mean $1$, and the censoring distribution is also exponential, but with a rate chosen to have a certain percentage of censored events. To get all our distributions on the same scale, let's transform variables $X$ and $C_X$ on using $F_X(\cdot)$ and transform variables, $Y$ and $C_Y$ on using $F_Y(\cdot)$. So for $X$ and $Y$ we end up having variables $U$ and $V$ that are uniformly distributed, and for $C_X$ and $C_Y$ we get $Z = F_X(C_X) =  1 - e^{-C_X}$ and $W = F_Y(C_Y)$ the following way. Suppose $C_X \sim 1-e^{-\beta_X \cdot c_X}$, then:
{\scriptsize{
	$$
	\begin{aligned}
    Pr \{ Z < z \} = Pr \{ 1 - e^{-C_X} < z \} = Pr \{ C_X < -ln(1 - z)\} = F_{C_X}(-ln(1 - z)) = 1-e^{-\beta (-ln(1 - z))} = 1 - (1-z)^{\beta}
	\end{aligned}
	$$
}}
Which means that both, $Z$ and $W$ have beta distribution with $\alpha_Z = \alpha_W = 1$ and $\beta_Z = \beta_X$, $\beta_W = \beta_Y$.

The question is if can evaluate the expression for Spearman's correlation under random censoring. We should probably start with conditional covariance and try to do something with  \emph{Fr\'echet-Hoeffding bounds}

{\scriptsize{
	$$
	\begin{aligned}
    max(u + v - 1, 0) \leq H(u, v) \leq min(u, v)
	\end{aligned}
	$$
}}
Where $H(u,v)$ is a copula in our notations.
...
\cite{fan2000dependence} noted that the majority of clinical studies are conducted over a limited time period. If the  question of the study is association of two survival variables, such studies allow to compute association only over the period of the study. It is also possible that researchers might be interested in association of two survival variables during different time periods of the study. In order to address this need, \cite{fan2000dependence} suggested two statistics, a limited region cross ratio and a limited region Kendall's tau. Both statistics are suitable for right-censored data and consistent and asymptotically normally distributed. The authors further developed the idea of \cite{oakes1989bivariate} who proposed a local Kendall's $\tau$: $\tau(t_X, t_Y) = E\{sign(T_{X,1} - T_{X,2})(T_{Y,1} - T_{Y,2})~|~ min(T_{X,1},T_{X,2}) = t_X ~and~ min(T_{Y,1},T_{Y,2}) = t_Y \}$, where $(T_{X,1}, T_{X,2})$ and $(T_{Y,1}, T_{Y,2})$ are two independent pairs. Oakes incorrectly suggested that his derivations work for both continuous and discrete cases. We correct his derivations and the derivations of \cite{fan2000dependence} for limited region Kendall's tau in the discrete (see earlier sections) case and propose a Spearman's correlation over a limited time region $[0,t_X] \times [0, t_y]$.\\

One way of defining Spearman's correlation is the probability of concordance minus probability of discordance for three independent pairs $(T_{X,0}, T_{Y,0})$, $(T_{X,1}, T_{Y,1})$, and $(T_{X,2}, T_{Y,2})$:
$$
\begin{aligned}
  &P((T_{X,0}-T_{X,1})(T_{Y,0} - T_{Y,2})>0) - P((T_{X,0}-T_{X,1})(T_{Y,0}-T_{Y,2})<0) = E\{sign(T_{X,1} - T_{X,2})(T_{Y,1} - T_{Y,2}) \}. 
\end{aligned}
$$

We switch notations here:
  $$
  \begin{aligned}
...
Now, let's find the joint distribution of concordance and discordance and of $X_{1}\wedge X_{2} = x,~Y_{1}\wedge Y_{2} = y$:
	$$
	\begin{aligned}
		P\{ (X_0>X_1, Y_0>Y_2) &\cap (X_{0}\wedge X_{1} = x,~Y_{0}\wedge Y_{2} = y) \} =P\{ X_0>x, X_1=x, Y_0>y, Y_2=y)\} = S(x, y)F(dx)G(dy) \\
		P\{ (X_0<X_1, Y_0<Y_2) &\cap (X_{0}\wedge X_{1} = x,~Y_{0}\wedge Y_{2} = y) \} =P\{ X_0=x, X_1>x, Y_0=y, Y_2>y)\} = S(dx, dy)F(x)G(y) \\
		P\{ (X_0>X_1, Y_0<Y_2) &\cap (X_{0}\wedge X_{1} = x,~Y_{0}\wedge Y_{2} = y) \} =P\{ X_0>x, X_1=x, Y_0=y, Y_2>y)\} = S(x, dy)F(dx)G(y) \\
		P\{ (X_0<X_1, Y_0>Y_2) &\cap (X_{0}\wedge X_{1} = x,~Y_{0}\wedge Y_{2} = y) \} =P\{ X_0=x, X_1>x, Y_0>y, Y_2=y)\} = S(dx, y)F(x)G(dy) \\
	\end{aligned}
	$$
 Therefore 
	$$
...
If we weight $\rho(x, y)$ by $\frac{h(x, y)}{\int_0^x \int_0^y h(x, y)}$ and integrate the above expression over $[0,x]\times[0,y]$, we get a \emph{finite region Spearman} $\rho$:
	$$
	\begin{aligned}
...
To see if this is the same as Spearman $\rho$, let's express the correlation of PSR in terms of $S(x,y)$, $F(x)$, and $G(y)$:
	$$
	\begin{aligned}
		P_c - P_d &= P\left\{ (X_1 - X)(Y_2 - Y) > 0 \right\} -  P\left\{ (X_1 - X)(Y_2 - Y) < 0 \right\} = \\
    &\text{skipping some algebra, see Qi's paper}\\
...
or, in terms of survival functions:
{\small{
\begin{equation}
		\rho_{\text{PSR}} = \int_0^{+\infty} \int_0^{+\infty} \left\{ \left[1 - 2S_X(x^-) + S_X(dx)\right]  \left[1 - 2S_Y(y^-) + S_Y(dy)  \right]  \right\}S(dx,dy) = \\
\end{equation}
}}

Whether the two expressions for correlation of PSR are equivalent or not is still a question (work in progress). A small simulation study showed that they seem to be equivalent in the absence of censoring, but when the data is censored they are not equivalent. My current guess is that,  in general, they are not equivalent. In favor of his guess is the fact that the equation (\ref{SpearmanRhoSurvFunc}) was obtained by fixing values $min(X,X_1) = x$ and $min(Y,Y_1) = y$, while equation (\ref{PSRrhoSurvFunc}) was obtained by fixing values $X=x$ and $Y=y$.

\subsection{Yet another way of computing Spearman's correlation}
As usual we have three pairs $(T_{X,0}, T_{Y,0})$, $(T_{X,1}, T_{Y,1})$, and $(T_{X,2}, T_{Y,2})$. Previously, we fixed $(T_{X,0}, T_{Y,0})$ and went from their. Now, we want to try to fix  $(T_{X,1}, T_{Y,1})$ and $(T_{X,2}, T_{Y,2})$:

{\normalsize{
$$
\begin{aligned}
  \rho = &P((T_{X,0} - T_{X,1})(T_{Y,0} - T_{Y,2})>0 )
   - P((T_{X,0} - T_{X,1})(T_{Y,0} - T_{Y,2})<0 ) = \\
   =  &P(T_{X,0} - T_{X,1} > 0, ~ T_{Y,0} - T_{Y,2}>0 ~or~ T_{X,0} - T_{X,1} < 0, ~ T_{Y,0} - T_{Y,2} < 0~|~T_{X,1}, T_{Y,2})\cdot P(T_{X,1})P(T_{Y,2})\\
    &- P(T_{X,0} - T_{X,1}<0,~ T_{Y,0} - T_{Y,2}>0 ~or~ T_{X,0} - T_{X,1}>0,~ T_{Y,0} - T_{Y,2}<0 ~|~T_{X,1}, T_{Y,2})\cdot P(T_{X,1})P(T_{Y,2}) = \\
  %%%
   =&  [  S_{X,Y}(T_{X,1}, ~ T_{Y,2}) +  1 - S_{X,Y}(T_{X,1}^-, 0) -  S_{X,Y}(0, ~ T_{Y,2}^-)  +  S_{X,Y}(T_{X,1}^-, ~ T_{Y,2}^-)   ]\cdot S_X(dT_{X,1})S_Y(dT_{Y,2})\\
    &- [  S_{X,Y}(0, T_{Y,2}) - S_{X,Y}(T_{X,1}^-, T_{Y,2})  +  S_{X,Y}(T_{X,1}, 0) - S_{X,Y}(T_{X,1}, T_{Y,2}^-)  ]\cdot S_X(dT_{X,1})S_Y(dT_{Y,2}) \\
  %%%
\end{aligned}
$$
}}
For continuous case, this looks like this:
{\normalsize{
$$
\begin{aligned}
  \rho = &[  4\cdot S_{X,Y}(T_{X,1}, ~ T_{Y,2}) - 2\cdot S_{X,Y}(T_{X,1}, 0) -  2 \cdot  S_{X,Y}(0, ~ T_{Y,2})  +  1  ]\cdot S_X(dT_{X,1})S_Y(dT_{Y,2})\\
  %%%
\end{aligned}
$$
}}
...
\section{Adjusted Spearman's correlation}
\textbf{Quale, van der Laan, Robins, 2006, Locally efficient estimation with bivariate right-censored data, file: Quale\_VanDerLaan\_Robins\_2006.pdf}
Authors propose a locally efficient estimator that is \emph{made} out of two estimators: 1) $F_n$ lower dimensional parametric estimator of events, 2) $G_n$ consistent estimator of censoring events. It can be the other way around: $F_n$ is consistent and $G_n$ does not have to be. The resulting estimator is consistent and very efficient (compared to Dabrowska's estimator).\\

\textbf{Yang, 2005, A NEW NONPARAMETRIC BIVARIATE SURVIVAL FUNCTION ESTIMATOR UNDER RANDOM RIGHT CENSORING, file: yang\_bivarSurf\_2005.pdf}
It is an interesting estimator, it involves a band width, but no comparison to other estimators. The introduction is a thorough review of the literature.

\textbf{Li, Lin, 2006, Semiparametric Normal Transformation Models for Spatially Correlated Survival Data} 

\textbf{Li, Prentice, Lin, 2008, Semiparametric maximum likelihood estimation in normal transformation models for bivariate survival data}

Read Hsu, Prentice (1996)\\
Cai and Prentice (1997) modeled $\lambda_{ki}(t) = Y_{ki}(t)\lambda_0(t) e^{\beta^T Z_{ki}(t)}$ for each time point $i$ and for each cluster $k$. This is kind of simple, because both the $\beta$-coefficients and baseline hazard are the same for all members of the cluster.
...
\subsection{My plan}
\begin{enumerate}[1)]
  \item Look into bias of Kaplan - Meier function
  \item look into proof of consistency of limited region Kendall's $\tau$
  \item keep looking into adjusted bivariate survival models 
...
So the resulting variance is:
    ...
    &=\frac{\cancel{n}(\pmb{A}_n)^{-1}(\pmb{X}, \pmb{Y}, \pmb{Z}, \hat{\pmb{\theta}})\frac{1}{\cancel{n}} \pmb{B}_n(\pmb{X}, \pmb{Y}, \pmb{Z}, \hat{\pmb{\theta}})  \cancel{n}\left\{(\pmb{A}_n)^{-1}(\pmb{X}, \pmb{Y}, \pmb{Z}, \hat{\pmb{\theta}}) \right\}^T}{\cancel{n}} = \\
...
Therefore
  ...
  \begin{aligned}
    ...
    %%%
    %& = \left( e^{-\Lambda(y_i)e^{\pmb{z}_i\pmb{\beta}}} \right) e^{\pmb{z}_i\pmb{\beta}} (t_{(k)} - t_{(k-1)})  ~~~~  &i,k:~~~y_i = t_{(k)}\\
    %%%
      &= 0 ~~~~  &\text{otherwise} \\
  \end{aligned}
  $$
...
%     \frac{\partial PSR_{Y_i}}{\partial \beta_j} & =   \left( e^{-\Lambda(y_i)e^{\pmb{z}_i}\pmb{\beta}}  \Lambda(y_i) + \delta_i e^{-\Lambda(y_i^-)e^{\pmb{z}_i}\pmb{\beta}}  \Lambda(y_i^-) \right) \cdot e^{\pmb{z}_i\pmb{\beta}}  \cdot z_{ij} \\
%     %%%
%     \frac{\partial PSR_{Y_i}}{\partial \Lambda_{(k)}} & = \left( e^{-\Lambda(y_i)e^{\pmb{z}_i\pmb{\beta}}}  \right) e^{\pmb{z}_i\pmb{\beta}}  \cdot I_{(t_{(k)} \leq y_i <  t_{(k+1)})}
%     %%%
%   \end{aligned}
%   $$
%
%
% If we assumed continuity, our derivatives could be a little different. \texttt{R} function \texttt{coxph()} provides us with martingale residuals, $m(y_i)$, so we would like to use them in our computations. Using equality (\ref{survFunForCoxModelThroughMartRes}) we have
%
%   $$
%   \begin{aligned}
%     \frac{\partial PSR_{Y_i}}{\partial \beta_j} & =  (1+\delta_i)  \left( \delta_i   -  m(y_i)  \right) e^{m(y_i) - \delta_i}   \cdot z_{ij} \\
%     %%%
%     \frac{\partial PSR_{Y_i}}{\partial \Lambda_{(k)}} & = (1+\delta_i)   e^{m(y_i) - \delta_i} e^{\pmb{z}_i\pmb{\beta}} \cdot I_{(t_{(k)} \leq y_i <  t_{(k+1)})}
%   \end{aligned}
%   $$
...
% % \begin{equation}
% %   S(y_i) =  e^{-\Lambda(y_i) e^{\pmb{z}_i\pmb{\beta}}}
% %   \label(survFunForCoxModel)
% % \end{equation}
%
% \begin{equation}
%     \hat{m}(y_i) = \epsilon_i + log\left(\hat{S}(y_i) \right)
%     \label{martResCoxModel}
% \end{equation}
%
% \begin{equation}
%     S(y_i) =  e^{-\Lambda(y_i) e^{\pmb{z}_i\pmb{\beta}}}
...
%
% \begin{equation}
%     \hat{S}(y_i) = e^{\hat{m}(y_i) - \epsilon_i}
%     \label{survFunForCoxModelThroughMartRes}
% \end{equation}
%
% \begin{equation}
%     \Lambda(y_i) e^{\pmb{z}_i\pmb{\beta}} = -log\left(S(y_i)\right)
%     \label{baseHazForCoxModelThroughSurvFun}
% \end{equation}
%
% \begin{equation}
%     \Lambda(y_i)  = \left( \epsilon_i -  \hat{m}(y_i)  \right)\cdot e^{-\pmb{z}_i \pmb{\beta}}
%     \label{baseHazForCoxModelThroughMartRes}
% \end{equation}
%
%


% \subsection{M-estimation for \emph{Cox} survival model  using Stute's results. Hazard Rate as a Parameter.}
...
%
% Before we continue, let's remind ourselves that in the context of Cox model, with $\hat{m}(y_i)$ being martingale residuals, the following identities hold:
%
% % \begin{equation}
% %   S(y_i) =  e^{-\Lambda(y_i) e^{\pmb{z}_i\pmb{\beta}}}
% %   \label(survFunForCoxModel)
...
%
%
% We denote ordered unique failure times as $t_{(k)}$. We can treat baseline hazard parameters either as instantaneous hazard rate $\lambda_{(k)}$ or cumulative hazard $\Lambda_{(k)}$. In this section, we choose hazard rates $\lambda_{(k)}$. \cite{stute1995central} derived estimating equations for the following integral, $\int \phi(x) F(dx)$, where $\phi(x)$ is a function with a property $\int \phi^2(x) F(dx) < \infty$, and $F(x)$ is a Kaplan-Meier estimate of the survival function. We know that cumulative hazard for continuous case can be written as: $\int \frac{F(dx)}{1 - F(x)}$. Parameter $\lambda_{(k)}$ can be expressed as $\int_{-\infty}^{t_{(k)}} \frac{F(dx)}{1 - F(x)}  -  \int_{-\infty}^{t_{(k-1)}} \frac{F(dx)}{1 - F(x)} = \int_{-\infty}^{+\infty}   I_{(t_{(k-1)}<x \leq t_{(k)})}  \frac{F(dx)}{1 - F(x)}$. So in order to use the result of Stute, we set $\phi(x) = \frac{I_{(t_{(k-1)}<x \leq t_{(k)})}}{1-F(x)} $. Note that although condition $\int \phi^2(x) F(dx) < \infty$ might not hold for this $\phi(x)$, this condition is sufficient and not necessary. Also, $F(x) = 1$ is an extreme case that is usually not achievable for continuous data, and we will try to watch for it in our computations.\\
%
% Similar to the previous section, we proceed by defining the estimating equations derived by Stute. Keeping in mind that for Cox regression $\hat{\Lambda}_{(j)}(y_i) = (\epsilon_i - \hat{m}(y_i))e^{\pmb{z}_i\pmb{\beta}}$, where $\hat{m}(y_i)$ is martingale residuals, we have:
%
% $$
%   \begin{aligned}
...
%   \end{aligned}
% $$
% Where $\hat{\lambda}_{(1)}(y_i)$ is a Nelson-Aalen estimator of the baseline hazard rate and:
%
% $$
%   \begin{aligned}
%     V_{j,i} &= \phi_j(y_i) \gamma_0(Y_i) \epsilon_i + \gamma_{j1}(y_i)(1 - \epsilon_i) - \gamma_{j2}(Y_i)\\
%     \phi_j(y_i) &= \frac{I_{(t_{(k-1)}<y_i \leq t_{(k)})}}{1-F(y_i)} \\
%   \end{aligned}
% $$
%
% Where $\gamma_0(y_i)$, $\gamma_{j,1}(y_i)$, $\gamma_{j,2}(y_i)$, $H^0(y)$, $H^1(y)$, $H(y)$ are the same as in section \ref{mEstForPSRUsingKM}. In order to compute $\phi_j(y_i)$, we need to figure out $\hat{F}(y_i)$, let's do this in the following way: $\hat{F}(y_i) = 1 - \hat{S}_{baseline}(y_i) =  1 - e^{-\hat{\Lambda}(y_i)}$; and therefore: $\phi_j(y_i) = \frac{I_{(t_{(k-1)}<y_i \leq t_{(k)})}}{e^{-\hat{\Lambda}_{(j)}(y_i)}}$. To summarize, our estimating equations will look like this:
%
%   \begin{equation}
%     \psi_{Y}(\hat{\Lambda}_{(j)}(y_i), y_i) = I_{(t_{(k-1)}<y_i \leq t_{(k)})}  e^{\hat{\Lambda}_{(j)}(y_i)} \cdot \gamma_0(y_i) \epsilon_i + \gamma_{j1}(y_i)(1 - \epsilon_i) - \gamma_{j2}(y_i) - \hat{\lambda}_{(j)}(y_i)\\
%     \label{estEqForHazUsingStute}
%   \end{equation}
%
% Now, let's focus on defining matrix $\pmb{A}_{Y,2,2}$.
% In order not to loose track of what we are doing, let's consider an example with only had one $\beta_l$ and two unique failure points, $t_1$ and $t_2$ (it's unrealistic, but saves us some space). The resulting $\pmb{A}_{Y,2,2}$ matrix (see section \ref{Defining:Matr} for  $\pmb{A}_{Y,2,2}$ definition) should look like this:
%
%   \[
%   \pmb{A}_{Y,2,2} = -\sum_i^n\begin{bmatrix}
%       \frac{\partial \psi_{Yi,\beta_l}}{ \partial \beta_l} &
%       \frac{\partial \psi_{Yi,\beta_l}}{ \partial \lambda_{(1)}} &
%       \frac{\partial \psi_{Yi,\beta_l}}{ \partial \lambda_{(2)}} \\
%       %%%%%%%%%
%       \frac{\partial \psi_{Yi,\lambda_{(1)}}}{ \partial \beta_l} &
%       \frac{\partial \psi_{Yi,\lambda_{(1)}}}{ \partial \lambda_{(1)}} &
%       \frac{\partial \psi_{Yi,\lambda_{(1)}}}{ \partial \lambda_{(2)}} \\
%       %%%%%%%%%
%       \frac{\partial \psi_{Yi,\lambda_{(2)}}}{ \partial \beta_l} &
%       \frac{\partial \psi_{Yi,\lambda_{(2)}}}{ \partial \lambda_{(1)}} &
%       \frac{\partial \psi_{Yi,\lambda_{(2)}}}{ \partial \lambda_{(2)}}\\
%   \end{bmatrix}
%   \]
%
% We need to know $\frac{\partial \psi_{Y}(\hat{\lambda}_{(j)}(y_i), y_i)}{ \partial \hat{\lambda}_{(k)}(y_i)}$, $\frac{\partial \psi_{Y}(\hat{\lambda}_{(j)}(y_i), y_i)}{ \partial \beta_l}$. From equation (\ref{estEqForHazUsingStute}) we have:
%
% \begin{equation}
%   \frac{\partial \psi_{Y_i,\lambda_{(j)}}}{  \partial \lambda_{(j)}} = \left(   e^{\hat{\Lambda}_{(j)}(y_i)}  I_{(t_{(k-1)}<y_i \leq t_{(k)})}  \cdot \gamma_0(y_i) \epsilon_i  \right) - 1
%   \label{derivOfStuteEstEqByHaz}
% \end{equation}
%
%
% As for the rest of the derivatives, we have:
%
% $$
% \begin{aligned}
%   %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%   \frac{\partial \psi_{Y_i,\beta_l}}{ \partial \lambda_{(j)}} &=  \frac{\partial \psi_{Y_i,\lambda_{(j)}}}{ \partial \beta_l} = \frac{\partial \psi_{Y_i,\lambda_{(1)}}}{  \partial \lambda_{(2)}} = \frac{\partial \psi_{Y_i,\lambda_{(2)}}}{  \partial \lambda_{(1)}} = 0\\
%   %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%   \frac{\partial \psi_{Y_i,\beta_l}}{ \partial \beta_l} &\text{~is provided by }\texttt{coxph()}\\
% \end{aligned}
% $$
% As a result:
% {\small{
...
%   \pmb{A}_{Y,2,2}
%    =-\begin{bmatrix}
%       \sum_i^n\frac{\partial \pmb{\psi}_{Yi,\beta_l}}{ \partial \beta_l} & 0 & 0 \\
%       %%%%%%%%%
%       0 & \sum_{i = 1}^n  \left(   e^{\hat{\Lambda}_{(1)}(y_i)}  I_{(0 <y_i \leq t_{(1)})}  \cdot \gamma_0(y_i) \epsilon_i  \right) - n & 0 \\
%       %%%%%%%%%
%       0 & 0 & \sum_{i = 1}^n  \left(   e^{\hat{\Lambda}_{(2)}(y_i)}  I_{(t_{(1)}<y_i \leq t_{(2)})}  \cdot \gamma_0(y_i) \epsilon_i  \right) - n \\
%   \end{bmatrix}
%   \]
% }}
%
...
% From the above, we anticipate a challenge of inverting matrix $\pmb{A}$. This is a challenge because if we have, for example, $n$ bivariate times to event, $(x_i,y_i)$, (where each $x_i$ and $y_i$ are unique) and suppose that each model has $p$ $\beta$ coefficients, then the dimension of matrix $\pmb{A}$ will be $2p + 2n + 6$. We can probably take advantage of the fact that the  largest square submatrices of $\pmb{A}_{X,1,1}$ and $\pmb{A}_{Y,2,2}$ are diagonal, and compute an overall $A^{-1}$ using these submatrices.\\
%
% In order to complete M-estimation, we still need to compute $\frac{\partial PSR_{Y_i}}{ \partial \hat{\Lambda}_{(k)}(y_i)}$ and $\frac{\partial PSR_{Y_i}}{ \partial \beta_l}$.
% We recall our definition: $PSR_{Y_i} =  F(y_i) + \epsilon_i F(y_{i}^-) - \epsilon_i  = \epsilon_i  - S(y_i) - \epsilon_i  S(y_i^-)$. So, we need to compute $\frac{\partial S(y_i)}{\partial \beta_i}$, $\frac{\partial S(y_i^-)}{\partial \beta_i}$, $\frac{\partial S(y_i)}{\partial \lambda_k}$, $\frac{\partial S(y_i^-)}{\partial \lambda_k}$. Keeping in mind that $S_{(j)}(y_i) =  e^{-\Lambda_{(j)}(y_i) e^{\pmb{z}_i\pmb{\beta}}}$, we have:
%
%
% $$
%   \begin{aligned}
%     \frac{\partial S_{(j)}(y_i, \pmb{\beta})}{\partial \beta_l} &=   -\hat{\Lambda}_{(j)}(y_i)  \cdot   e^{-\hat{\Lambda}_{(j)}(y_i) \cdot e^{\pmb{z}_i\pmb{\beta}}}  \cdot  e^{\pmb{z}_i\pmb{\beta}}   \cdot z_{il} \\
%     %%%
%     \frac{\partial S_{(j)}(y_i^-, \pmb{\beta})}{\partial \beta_l} &=   -\hat{\Lambda}_{(j)}(y_i^-)  \cdot   e^{-\hat{\Lambda}_{(j)}(y_i^-) \cdot e^{\pmb{z}_i\pmb{\beta}}}  \cdot  e^{\pmb{z}_i\pmb{\beta}}   \cdot z_{il}\\
%     %%%
%     \frac{\partial S_{(j)}(y_i, \pmb{\beta})}{\partial \Lambda_{(k)}(y_i)}  &=   -e^{-\hat{\Lambda}(y_i) e^{\pmb{z}_i\pmb{\beta}}}  \cdot  e^{\pmb{z}_i\pmb{\beta}}  \cdot I_{  \left( \hat{\Lambda}_{(j)} = \hat{\Lambda}(y_i, \pmb{\beta})  \right)}\\
%     %%%
%     \frac{\partial S_{(j)}(y_i^-, \pmb{\beta})}{\partial \Lambda_{(k)}(y_i)}  &=   -e^{-\hat{\Lambda}(y_i^-) e^{\pmb{z}_i\pmb{\beta}}}  \cdot  e^{\pmb{z}_i\pmb{\beta}}  \cdot I_{  \left( \hat{\Lambda}_{(j)} = \hat{\Lambda}(y_i^-, \pmb{\beta})  \right)}\\
%   \end{aligned}
%   $$
...
% $$
%   \begin{aligned}
%     \frac{\partial \pmb{PSR}_{Y_i}}{\partial \beta_l} &= \left\{    \hat{\Lambda}_{(j)}(y_i)  \cdot   e^{-\hat{\Lambda}_{(j)}(y_i) \cdot e^{\pmb{z}_i\pmb{\beta}}}  +   \epsilon_i \hat{\Lambda}_{(j)}(y_i^-)  \cdot   e^{-\hat{\Lambda}_{(j)}(y_i^-) \cdot e^{\pmb{z}_i\pmb{\beta}}}   \right\} \cdot  e^{\pmb{z}_i\pmb{\beta}}   \cdot z_{il} \\
%     %%%%%%%%%%%
%     \frac{\partial \pmb{PSR}_{Y_i}}{\Lambda_{(j)}} &= -\frac{\partial \left[  \hat{S}_Y(y_i) + \hat{S}_Y(y_{i-1}) \epsilon_i - \epsilon_i  \right]}{\partial \Lambda_{(j)}} = e^{-\hat{\Lambda}(y_i) e^{\pmb{z}_i\pmb{\beta}}}  \cdot  e^{\pmb{z}_i\pmb{\beta}}  ,&~~~~\text{if}~\hat{\Lambda}_{(j)} = \hat{\Lambda}(y_i, \pmb{\beta})\\
%     &=\epsilon_i e^{-\hat{\Lambda}(y_i^-) e^{\pmb{z}_i\pmb{\beta}}}  \cdot  e^{\pmb{z}_i\pmb{\beta}}  ,&~~~~\text{if}~\hat{\Lambda}_{(j)} = \hat{\Lambda}(y_i^-, \pmb{\beta})\\
%     &=0,&~~~~\text{otherwise}
%   \end{aligned}
% $$
%
...
% $$
%   \begin{aligned}
%     \frac{\partial PSR_{Y_i}}{\partial \beta_l} &=  -\left\{  log\left(S(y_i)\right) \cdot  S(y_i) + \epsilon_i log\left(S(y_i^-)\right) \cdot  S(y_i^-) \right\}   \cdot  z_{il} \\
%     \frac{\partial PSR_{Y_i}}{\partial \Lambda_{(j)}} &=   \left\{    S(y_i)\cdot  I_{(\hat{\Lambda}_{(j)} = \hat{\Lambda}(y_i, \pmb{\beta}))}  + \epsilon_i   S(y_i^-)   \cdot I_{(\hat{\Lambda}_{(j)} = \hat{\Lambda}(y_i^-, \pmb{\beta}))} \right\}   \cdot  e^{\pmb{z}_i\pmb{\beta}} \\
%   \end{aligned}
% $$
%
%
% \texttt{R} function \texttt{coxph()} provides us with martingale residuals, so we would like to use them in our computations. Using equality (\ref{survFunForCoxModelThroughMartRes}) we have:
% $$
%   \begin{aligned}
%     \frac{\partial PSR_{Y_i}}{\partial \beta_l} &=  -\left\{  (m(y_i) - \epsilon_i)  \cdot  e^{\hat{m}(y_i) - \epsilon_i} + \epsilon_i    (m(y_i^-) - \epsilon_{i-1})  \cdot  e^{\hat{m}(y_i^-) - \epsilon_{i-1}}   \right\}   \cdot  z_{il} \\
%     \frac{\partial PSR_{Y_i}}{\partial \Lambda_{(j)}} &=   \left\{    e^{\hat{m}(y_i) - \epsilon_i}  \cdot  I_{(\hat{\Lambda}_{(j)} = \hat{\Lambda}(y_i, \pmb{\beta}))}  + \epsilon_i   e^{\hat{m}(y_i^-) - \epsilon_{i-1}}   \cdot I_{(\hat{\Lambda}_{(j)} = \hat{\Lambda}(y_i^-, \pmb{\beta}))} \right\}   \cdot  e^{\pmb{z}_i\pmb{\beta}} \\
%   \end{aligned}
% $$
  
% \subsection{M-estimation for \emph{Cox} survival model  using Stute's results. Cumulative Hazard as a Parameter.}
% M-estimation for Cox regression is complicated because Cox regression provides score equations only for $\hat{\pmb{\beta}}$. We, however also need score equations for baseline hazard parameters. \\
%
% Before we continue, let's remind ourselves that in the context of Cox model, with $\hat{m}(y_i)$ being martingale residuals, the following identities hold:
%
...
%
%   \begin{equation}
%     \psi_{Y}(\hat{\Lambda}_{(j)}(y_i), y_i) = I_{\{ y_i \leq t_j \}}e^{\hat{\Lambda}_{(j)}(y_i)} \cdot \gamma_0(y_i) \epsilon_i + \gamma_{j1}(y_i)(1 - \epsilon_i) - \gamma_{j2}(y_i) - \hat{\Lambda}_{(j)}(y_i)\\
%     \label{estEqForHazUsingStute}
%   \end{equation}
%
% Now, let's focus on defining matrix $\pmb{A}_{Y,2,2}$.
...
%
% \begin{equation}
%   \frac{\partial \psi_{Y_i,\Lambda_{(j)}}}{  \partial \Lambda_{(j)}} = \left(   e^{\hat{\Lambda}_{(j)}(y_i)}  I_{\{ y_i \leq t_j \}} \cdot \gamma_0(y_i) \epsilon_i  \right) - 1
%   \label{derivOfStuteEstEqByHaz}
% \end{equation}
%
% In order to make the computation of equations (\ref{estEqForHazUsingStute}) and (\ref{derivOfStuteEstEqByHaz}) easier, we rewrite them using (\ref{baseHazForCoxModelThroughMartRes}):
%
%
% $$
% \begin{aligned}
%     \psi_{Y}(\hat{\Lambda}_{(j)}(y_i), y_i) &= e^{ (\epsilon_i  -  \hat{m}(y_i)) e^{-\pmb{z}_i\pmb{\beta}}    }\cdot I_{\{ y_i \leq t_j \}}  \cdot \gamma_0(y_i) \epsilon_i + \gamma_{j1}(y_i)(1 - \epsilon_i) - \gamma_{j2}(y_i) - (\epsilon_i  -  \hat{m}(y_i)) e^{-\pmb{z}_i\pmb{\beta}} \\
%     %%%%%%%%%
%   \frac{\partial \psi_{Y_i,\Lambda_{(j)}}}{  \partial \Lambda_{(j)}} &= \left(   e^{ (\epsilon_i  -  \hat{m}(y_i)) e^{-\pmb{z}_i\pmb{\beta}}    } \cdot I_{\{ y_i \leq t_j \}} \cdot \gamma_0(y_i) \epsilon_i  \right) - 1
% \end{aligned}
% $$
%
% As for the rest of the derivatives, we have:
...
%       \sum_i^n\frac{\partial \pmb{\psi}_{Yi,\beta_l}}{ \partial \beta_l} & 0 & 0 \\
%       %%%%%%%%%
%       0 & \sum_{i = 1}^n  \left(    e^{ (\epsilon_i  -  \hat{m}(y_i)) e^{-\pmb{z}_i\pmb{\beta}}}    I_{\{ y_i \leq t_{(1)} \}} \cdot \gamma_0(y_i) \epsilon_i  \right) - n & 0 \\
%       %%%%%%%%%
%       0 & 0 & \sum_{i = 1}^n  \left(    e^{ (\epsilon_i  -  \hat{m}(y_i)) e^{-\pmb{z}_i\pmb{\beta}}}  I_{\{ y_i \leq t_{(2)} \}} \cdot \gamma_0(y_i) \epsilon_i  \right) - n \\
%   \end{bmatrix}
%   \]
% }}
%
...
%   \]
%
% %And we remind ourselves that $\mathcal{R}(t_{(k)}) = \left\{y_l: ~y_l \geq t_{(k)} ~\cap~ ~y_i=t_{(k)}\right\}$.
...
\begin{enumerate}
  \item Note that we have (or do we?) $T|Z~~ \perp  ~~C|Z$
  \item Frank was saying something about taking a copula and doing a likelihood function then comparing our stuff to the result of the likelihood.
  \item Frank has also suggested looking at the Hoeffding Dependence Coefficient (see \url{http://support.sas.com/documentation/cdl/en/procstat/67528/HTML/default/viewer.htm#procstat_corr_details07.htm})
  \item Dandan has suggested to look into \emph{sudo values} approach of imputing the data so it is not censored anymore
  \item Frank said that there is a rich kidney data (ask Leena)
  \item Frank suggested transitional models with multiples states (Prof. Thernau, the one who wrote the survival analysis book).
  \item Dandan has also pointed out the importance of the having a paper with a collaborator
\end{enumerate}

\section{Thoughts about the committee meeting}
1. I love our estimator, but what bothers me the most is that when the censoring is informative, it is possible for it to be biased away from zero. We haven't proved that the bias is always toward zero.
2. If we do what the committee wanted, the papers that Bryan has sent were very helpful.
I learned how to make my Dabrowska's estimator work faster (Prentice and Zhao, 2018):

$$
\begin{aligned}
  \hat{F}(t_{1,i}, t_{2,j}) &= \frac{  \hat{F}(t_{1,i}, t_{2,j}^-) \hat{F}(t_{1,i}^-, t_{2,j}) } { \hat{F}(t_{1,i}^-, t_{2,j}^-)  }   \left\{   \frac{  d^{00}_{ij} r_{ij}  }{    (d_{ij}^{10} + d_{ij}^{00})  (d_{ij}^{01} + d_{ij}^{00})}  \right\}\\
  &\text{where I don't understand some of the notations...} 
\end{aligned}
$$
The paper of Prentice (2014) provides a non-parametric estimate of survival surface without the problem of negative mass, although it does not improve the efficiency much. It uses the idea of self-consistency first introduced by Efron (1967) and later developed by of Tsai and Crowley (1985). Prentice suggested first to use Dabrowska estimator but excluding the points with double censoring from the interior risk region (while leaving them on the margins, I think). He suggests to estimate survival functions for both, events $F$ and censored events $G$ and to construct $\hat{F}^*$ and $\hat{G}^*$. Then, he suggested to estimate c.d.f of $\underline{F}$ and $\underline{G}$, which I did not understand how, but suppose I did, then we could have constructed our self-consistent estimator like this:

$$
\begin{aligned}
  \hat{F}(\Delta X_1, \Delta X_2) &=  \frac{\hat{F}^*}{1 - \underline{\hat{G}}(X_1^-, X_2^-)}, \text{ if $(X_1, X_2)$ is a failure time grid point}\\
  &=0,\text{   otherwise}\\
  \hat{G}(\Delta X_1, \Delta X_2) &=  \frac{\hat{G}^*}{1 - \underline{\hat{F}}(X_1, X_2)}, \text{ if $(X_1, X_2)$ is a censoring time grid point}\\
  &=0,\text{   otherwise}
\end{aligned}
$$
There is not analytic variance expression for the above estimator, but the bootstrap can be used. 

The paper of Li, Prentice, and Lin, shows how to do a semi-parametric model, in other words, to adjust for covariates. Their model uses a transformation for cumulative marginal hazards, so that the joint distribution of time to event variables becomes Normal, which means that it has a built-in correlation, Spearman's correlation. 
If I were to implement both of them, I would need to figure out the self-consistency estimation, and the variance estimation for unadjusted; and also I would have to figure out the semiparametric model, then I would have two papers!



\section{Future papers}
\begin{enumerate}
  \item applying the method to EHR data where we have two (or more) times to event: time to being genotyped for certain mutations after being prescribed a certain medication (Johnathan's data collected from electronic health records (EHR))
  \item Cumulative probability models (CPMs) for censored/detection limits. Qi has already done single detection limit)
  \item Extension of bivariate survival  (your estimator: limited region rho)
  \item Errors in variables with EHR data in the context of bivariate survival
  \item EHR correlation (Linjun's work, BioVU)
  \item How to use correlation to predict one bivariate outcome based on another
  \item How to predict one time to event based on another time to event
  \item Causal inference in bivariate survival 
\end{enumerate}
...
Right now, I am running power for local rho with bootstrap: 1) job 27296774 (clayton, 100);  2) job 27355092 (clayton, 200), 3) job XXX (clayton, 500);  4) job 27247087 (frank, 100);  5) job XX (frank, 200); 5) job XX (frank, 500).


\begin{enumerate}
  \item what do I do at the lastly censored observation ? KM?
  \item Try to evaluate the bias.
  \item Plot for conditional correlation: get rid of uncensored case and get rid of "True correlation"
  \item Do data analysis using a model and a conditional correlation
  
  \item list all methods for plot in unadjusted correlation

  \item include the simulation page: about copulas
  \item mention previous work
  \item goal: create spearman's rank because it is very good
  \item then do notations
  \item liu: show that spearman's = corr of the new type of residuals
  \item we follow similar approach in the context of PSRs
  \item up to now 3 minutes
  \item notaions 
  \item definition of Spearmans' corr of CDFs
  \item result of Liu about corr(psr, psrs)
  \item rank correlation for censored data
  \item time from HAART initiation
    
  \item $\bigvee$ For conditional correlation: get rid of exponential model.
  \item $\bigvee$ For conditional correlation: look at the bias and coverage (of multiple points). Also do the bias and coverage for the biased estimate, proportion of CIs that cover the truth 
  \item for Conditional correlation plots for CASANET data do plots with 3 knots.
  \item $\bigvee$ For all tables: split each column into two, put Clayton 0.2 and Frank 0.2 together
  \item $\bigvee$ for table 3, note that the bias is larger than in table 2
  \item $\bigvee$ for table 3 add another table for power.
  \item $\bigvee$ for table 3, put the independent censoring into the appendix
  \item $\bigvee$ table 2 drop column "Method". Get rid of "bootstrap". Write that the bias is toward zero.
  \item $\bigvee$: Question from Chun: Figures 3, 5: for some reason the middle columns are curved (but they have to be linear) Answer:  The reason why they are curved is that we first predict each $resid$, $resid^2$, $resid * resid$, and then we make a "correlation" out of it.
  \item $\bigvee$: bias for the population parameter under censoring instead of "bias of bias".
  \item $\bigvee$ For conditional correlation: we probably do not need the survival models to contain a quadratic term.
  \item $\bigvee$ Change the notations to Tx, Cx, deltax, Fx, X, Ty, Cy, Deltay, Fy
  \item...  
  \item $\bigvee$ Inside tabel 6 separate adults and children (partial and unadjusted correlation)
  \item $\bigvee$ re-run simulations with different seed for partial correlation, sample size 100, 200, because the seeds were a sequence from 1, 1000 and we need random seeds: (JOBID: 24699524, completed)
  \item $\bigvee$ run simulations with different seed for partial correlation, sample sizes 500 (JOBID: 24787776, had to adjust seeds b/c it would not run, completed)
  \item $\bigvee$ get simulations for bootstrap, frank, 100, 500 (JOBID: 24554302, completed)
  %\item get simulations for bootstrap, frank, 100, 500 (JOBID: 24787853, 24938538, still running)
  \item $\bigvee$ get simulations for bootstrap, frank, clayton, 200 (JOBID: 25021027 completed)
  \item find the literature on non-parametric survival models
      
  \item $\bigvee$ exclude "no censoring" from the first unadjsuted correlation, put no censoring option in Appendix
  \item $\bigvee$ sample - make it a first column and have only one number per sample size group.
  \item exclude "power" from table 2, put no censoring option in Appendix. The rest of the numbers: first: bias: (coverage); bias* (coverage*); censoring percent.
  \item $\bigvee$  table 3 should be moved into the Appendix, add text that for dependent censoring the results are slightly attenuated
  \item $\bigvee$ in table 4 rerun it with censoring scenario of .3 (approximately, JOBID for 500 is 25042640, completed; JOBID for 100, 200 is 25023321, completed)
  \item in table 4 why the bias is different - did not get the question
  \item \textbf{Next week: example of CASSANETT data}
  \item \textbf{the week after that: Conditional simulations}
  \item \textbf{the week after that: Discussion}
  \item \textbf{4/13} is our goal for paper submission
  \item schedule the committee meeting in April after submitting the paper.
  ...
  \item Look at papers that estimate $\rho$ using copulas: do they do Partial rho?
  ...
  %\item baseline hazard score equations: Stute The central limit theorem under random censorship. Bryan's number: 615-624-2220
