...
For exponential survival model, we assume that $\sigma = 1$, and:
  ...
  \begin{aligned}
    ...
    F_{e^{\epsilon_{i}}}(t) &= 1 - e^{-(e^{-\pmb{z}_i \pmb{\beta}})t}\\
    S_{e^{\epsilon_{i}}}(t) &= e^{-(e^{-\pmb{z}_i \pmb{\beta}})t}\\
    f_{e^{\epsilon_{i}}}(t) &= e^{-\left(\pmb{z}_i \pmb{\beta}\right)}e^{-(e^{-\pmb{z}_i \pmb{\beta}})t}\\
  \end{aligned}
  $$
The likelihood and the log-likelihood are the following:
  $$
  \begin{aligned}
    L &= \prod_{i=1}^N  \left[ e^{-\left(\pmb{z}_i \pmb{\beta}\right)}e^{-(e^{-\pmb{z}_i \pmb{\beta}})t_i} \right]^{\delta_i} \left[e^{-(e^{-\pmb{z}_i \pmb{\beta}})t_i}\right]^{1-\delta_i} =\prod_{i=1}^N   \left[e^{-\left(\pmb{z}_i \pmb{\beta}\right)}\right]^{\delta_i}   e^{-(e^{-\pmb{z}_i \pmb{\beta}})t_i} = \prod_{i=1}^N   e^{-\pmb{z}_i \pmb{\beta}\delta_i-(e^{-\pmb{z}_i \pmb{\beta}})t_i}\\
    l &= \sum_{i=1}^N  \left[  -\pmb{z}_i \pmb{\beta}\delta_i-(e^{-\pmb{z}_i \pmb{\beta}})t_i \right]
  \end{aligned}
  $$
If $\pmb{\beta} = (\beta_0, \beta_1, \beta_2)$ then:
  $$
  \begin{aligned}
    l &= \sum_{i=1}^N  \left[  -\beta_0\delta_i-\beta_1 z_{i1}\delta_i-\beta_2z_{i1}\delta_i-(e^{-\beta_0-\beta_1 z_{i1}-\beta_2z_{i1} })t_i \right]\\
    \frac{\partial l}{\partial \beta_0} &= \sum_{i=1}^N  \left[  -\delta_i+(e^{-\pmb{z}_i \pmb{\beta}})t_i \right]\\
    \frac{\partial l}{\partial \beta_1} &= \sum_{i=1}^N  \left[  -\delta_i z_{i1}+(e^{-\pmb{z}_i \pmb{\beta}})z_{i1}t_i \right]\\
    \frac{\partial l}{\partial \beta_2} &= \sum_{i=1}^N  \left[  -\delta_i z_{i2}+(e^{-\pmb{z}_i \pmb{\beta}})z_{i2}t_i \right]\\
  \end{aligned}
  $$
...
We only need to figure out the following derivatives: $\frac{\partial PSR_Y}{\partial \beta_0}$, $\frac{\partial PSR_Y}{\partial \beta_1}$, $\frac{\partial PSR_Y}{\partial \beta_2}$. Because $PSR = (1+\delta)F(t) - \delta = (1+\delta)(1 - S(t)) - \delta = 1 - (1+\delta)S(t)$, we have:
  $$
  \begin{aligned}
    PSR_{Y_i} &= 1 - (1+\delta_i)S_Y(t) = 1 - (1+\delta_i)e^{-(e^{-\pmb{z}_i \pmb{\beta}})t} \\
    \frac{\partial PSR_Y}{\partial \beta_0} = - (1+\delta_i)e^{-(e^{-\pmb{z}_i \pmb{\beta}})t}\\
  \end{aligned}
  $$
