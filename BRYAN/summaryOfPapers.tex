...
First, let's introduce some notations. Times to event for sons and fathers are $S$ and $T$ respectively with the corresponding distribution functions of $F(S)$ and $G(T)$. We fix two points $S_0$ and $T_0$ and find the covariance of PRS under the assumption of perfect dependence, for example, $S\equiv T$. For brevity, we denote $F(S)=U$, $G(T)=V$, $F(S_0)=U_0$, and $G(T_0)=V_0$.\\
~\\
Before we continue, let's make sure we understand the probability distribution function of $(S, T)$. From \cite{nelsen2007introduction} we know that the joint \emph{cdf} of these two variables is $min(F(S), G(T))$. This does not help to derive the joint \emph{pdf} though (at least this did not help me). Let's denote the joint \emph{pdf} as $h(u, v)$. In order to figure our the joint \emph{pdf}, let's use conditional principle, $h(u, v)=h_{U|V=v}(u|v)\cdot h_V(v)$. We know that $F(S)=U \sim Unif(0, 1)$. This means that the marginal \emph{pdf} of $U$ is uniform, $h_V(v)=I_{[0\leq U \leq 1]}$. The conditional $h_{U|V=v}$ equals $1$ when $U=v$ and $0$ when $U\neq v$, or, $h_{U|V=v} = I_{[0\leq V \leq 1]}\cdot I_{[U=v]}$. This means that  $h(u, v)= I_{[0\leq V \leq 1]}\cdot I_{[U=v]}\cdot I_{[0\leq U \leq 1]}$. It is hard to imagine, but $h(u, v)$ is strange function in a shape of a piece of a plain, perpendicular to the plain generated by $(U,V)$, and stretching from $(U,V) = (0, 0)$ to $(U,V) = (1,1)$. In order for this \emph{plain} to sum up to 1, it has to be divided by $\sqrt{2}$.\\
~\\
Because $h(u, v)$ is this strange plain, in order to find covariance, we cannot just take a regular integral, we have to take an integral on a curve $U=V$, or $V(t)=t$ and $U(t) = t$. We know that the integral on a curve has to have a term $\sqrt{\left(\frac{dU(t)}{dt}\right)^2 + \left(\frac{dV(t)}{dt}\right)^2}$. This term equals to $\sqrt{1^2 + 1^2} = \sqrt{2}$, therefore two terms of $\sqrt{2}$ cancel out in the following integrals.\\
~\\
Suppose now that we need to find the covariance of PSR for a fixed $(u_0, v_0)$. Let's assume a continuous case:
	$$
	\begin{aligned}
    U &= U^-,~~~V = V^-\\
...
Summing up all four integrals and taking into account the symmetry, we have an expression for spearman correlation of two identical variables, $S\equiv T$ conditional on the censoring point $(u_0, v_0)$:
	$$
	\begin{aligned}
