...
$$
  \begin{aligned}
    \psi_{Y}(\hat{F}_{1,Y}(y_i), y_i) &= V_{1,i,Y} - \hat{S}_{1,Y}(y_i)\\
    \psi_{Y}(\hat{F}_{2,Y}(y_i), y_i) &= V_{2,i,Y} - \hat{S}_{2,Y}(y_i)\\
    ...\\
    \psi_{Y}(\hat{F}_{k,Y}(y_i), y_i) &= V_{k,i,Y} - \hat{S}_{k,Y}(y_i)\\
  \end{aligned}
$$
Where:
...
$$
  ...
  \end{aligned}
$$
Where $\gamma_0(Y_i)$, $\gamma_{j,1}(Y_i)$, $\gamma_{j,2}(Y_i)$, $H^0(y)$, $H^1(y)$, $H(y)$ are the same as in section \ref{mEstForPSRUsingKM}.\\
We still need to define $ 1 - \hat{S_{baseline}(Y_i)}$:

$$
  \begin{aligned}
    \hat{F}(Y_i) = 1 - \hat{S}_{baseline}(Y_i) =  1 - e^{-\hat{\Lambda}(Y_i)}\\
  \end{aligned}
$$
