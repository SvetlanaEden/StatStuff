...
\item In Qi's code (function \texttt{corTS()}), matrices $\pmb{A}_{Y,2,2}$, $\pmb{A}_{Y, 4,2}$, $\pmb{A}_{Y,5,2}$, $\pmb{A}_{Y, 7,2}$ (see section \ref{Defining:Matr}) were multiplied by $-1$. I am planning to check if this she was using function \texttt{corTS()} in a way that this did not cause a problem.

\item Derivations in section \ref{norm:QiCode} were not verified.

\item \emph{Parametrization} of all parametric survival regressions was verified (\texttt{R} does not provide any documentation, so I used papers, online resources, and my own guesses, which were verified by simulations, the value of the likelihood, and similar techiniques).
...
\item Derivations if sections \ref{MEst:exponential}, \ref{MEst:weibull}, and \ref{MEst:lognormal}  were thoroughly verified using simulation, numeric derivatives, and other techniques.
