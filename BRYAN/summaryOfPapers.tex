...
After writing the simulation plan, I focused on the problem of generating confidence intervals for conditional correlation as a function of calendar time. The problem is that for each event, the cox model is very significant, but the A matrix (the matrix required for the M-estimation) is computationally singular. I tried several things. 1) Matrix inversion by parts (I could compute the inverse, but the product of matrix A and its inverse would not result in unit-diagonal matrix); 2) I tried computing confidence intervals assuming known hazard (the result was not great: strange confidence intervals); 3) I tried the generalized inverse (which I know, does not make sense), the confidence intervals were even greater; 4) Another and final attempt to address this problem was employing Stute's estimating equations, but Alas, they gave negative variance...

\section{Plan for tomorrow}
\begin{enumerate}
  \item look at the censoring probability (for both events) as a function of CD4 level
  \item generate nice plots for conditional correlation (simulated and real) plots
  \item look at the results of the simulations from ACCRE
  \item work on your code to incorporate bias into your table
  \item run simulations on ACCRE that include bias.
\end{enumerate}
