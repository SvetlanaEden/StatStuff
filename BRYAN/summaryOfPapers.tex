...
Let's summarize:
  ...
  \begin{aligned}
    ...
    %%%%%%%%%%%% deriv by gamma
    \left.\frac{\partial PSR_{Y_i}}{\partial \gamma}\right|_{\gamma = ln(\sigma)} &=~~(1+\delta_i)\left[    -(ln(y_i) - \pmb{z}_i\pmb{\beta})\cdot\phi_{St.N.}\left(  \frac{ln(y_i) - \pmb{z}_i\pmb{\beta}}{\sigma}  \right)   +  \Phi_{St.N.}\left(  \frac{ln(y_i) - \pmb{z}_i\pmb{\beta}}{\sigma}  \right) + F_Y(y_i)    \right]
  \end{aligned}
  $$
...
We compute $\frac{\partial PSR_{Y_i}}{\partial \beta_j}$ using martingale residuals provided by \texttt{coxph}: 
  ...
  \begin{aligned}
    ...
    \frac{\partial PSR_{Y_i}}{\partial \beta_j} &=  (1+\delta_i)  \hat{S}(y_i)  \cdot  e^{\pmb{z}_i \hat{\pmb{\beta}}}   \cdot z_{ij} =  (1+\delta_i)  e^{\hat{m}_i - \delta_i}  \cdot  e^{\pmb{z}_i  \hat{\pmb{\beta}}}   \cdot z_{ij}
  \end{aligned}
  $$
