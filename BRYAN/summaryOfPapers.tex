...
Where $\psi_X(X_i,Z_i; \pmb{\theta}_X)$ and $\psi_Y(Y_i,Z_i; \pmb{\theta}_Y)$ are score functions for models of association of $X\sim Z $ and $Y \sim Z$ respectively.\\
...
Where $F_{\pmb{\theta}_Y}(\cdot)$ is a \emph{cdf} of $Normal(\pmb{Z} \cdot \pmb{\theta}_Y , \sigma^2)$ and $\Phi(\cdot)$ is a \emph{cdf} of a standard normal, $Normal(\pmb{0}, 1)$. In order to find $\pmb{A}_{Y, 4,2}$, we have to find  $\frac{\partial PSR_Y(\pmb{\theta}_Y)}{\partial \beta_0}$, $\frac{\partial PSR_Y(\pmb{\theta}_Y)}{\partial \beta_1}$, $\frac{\partial PSR_Y(\pmb{\theta}_Y)}{\partial \beta_2}$, and $ \frac{\partial PSR_Y(\pmb{\theta}_Y)}{\partial \sigma}$:
  ...
  $$
  \begin{aligned}
    \frac{\partial PSR_{Y_i}(\pmb{\theta}_Y)}{\partial \beta_0} = -2f\left(\frac{\pmb{Y}_i - \pmb{Z}_i\pmb{\theta}_Y}{\sigma}\right)\frac{1}{\sigma}\\
    \frac{\partial PSR_{Y_i}(\pmb{\theta}_Y)}{\partial \beta_1} = -2f\left(\frac{\pmb{Y}_i - \pmb{Z}_i\pmb{\theta}_Y}{\sigma}\right)\frac{z_{i,1}}{\sigma}\\
    \frac{\partial PSR_{Y_i}(\pmb{\theta}_Y)}{\partial \beta_2} = -2f\left(\frac{\pmb{Y}_i - \pmb{Z}_i\pmb{\theta}_Y}{\sigma}\right)\frac{z_{i,2}}{\sigma}\\
    \frac{\partial PSR_{Y_i}(\pmb{\theta}_Y)}{\partial \sigma} = -2f\left(\frac{\pmb{Y}_i - \pmb{Z}_i\pmb{\theta}_Y}{\sigma}\right)\frac{\pmb{Y}_i - \pmb{Z}_i\pmb{\theta}_Y}{\sigma^2}\\
  \end{aligned}
  $$
 ...
 results in 
In addition to $\pmb{A}_{Y,11}$ and $\pmb{A}_{Y,21}$ specified above, Qi's function \texttt{lm.scores()} returns \texttt{dpresid.dtheta}, which is:
  $$
  \begin{aligned}
    -2\cdot \frac{f\left(  \frac{y_i - \hat{y}_i}{\hat{\sigma}}  \right) }{\hat{\sigma}}
  \end{aligned}
  $$
Where $f(\cdot)$ is a normal density. This expression is probably obtained as a derivative of \emph{PSR}: $-2\cdot F\left(  \frac{y_i - \hat{y}_i}{\hat{\sigma}}  \right) - 1$
  $$
  \begin{aligned}
