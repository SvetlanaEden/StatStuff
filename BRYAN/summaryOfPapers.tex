...
Here how we are planning to proceed. \textbf{First, we assume that every time point is unique!} \cite{breslow1974large} established limitting normality for the product limit (Kaplan-Meier) estimates. Because PSR is a function of product limit estimates, we can use delta method to derive variance estimate for this function. For convenience (because I don't want to be overwhelmed by a bunch of derivatives), I will use delta method twice. Using notations of section \ref{Defining:Matr}, the first delta method step is used to estimate the variance-covariance matrix for $\hat{\theta}_1$, $\hat{\theta}_2$, $\hat{\theta}_3$, $\hat{\theta}_4$, and $\hat{\theta}_5$. The second delta method step is used to estimate $\rho_S = \frac{\theta_3 - \theta_1 \theta_2}{\sqrt{\theta_4 - \theta_1^2}\sqrt{\theta_4 - \theta_2^2}}$. Let's proceed with the first delta method step. Let's further denote a Kaplan-Meier estimate of $F_X(x)$ in point $x_i$ as $\hat{F}_X(x_i)$. In the context of this kind of estimation, $\hat{F}_X(x_i^-) = \hat{F}_X(x_{i-1})$, so we can write the following: $\widehat{PSR}_{X_i} = \hat{F}_X(x_i) + \hat{F}_X(x_{i-1})\delta_i - \delta_i$. We remember that each $\hat{F}_X(x_i)$ is a random parameter estimate and proceed to write:

{\footnotesize{  
$$
  \begin{aligned}
    g_1(\pmb{PSR}_{X_i}, \pmb{PSR}_{Y_i}) &= \sum_{i=1}^N PSR_{X_i} = \sum_{i=1}^N \left[  \hat{F}_X(x_i) + \hat{F}_X(x_{i-1})\delta_i - \delta_i   \right]\\
    g_2(\pmb{PSR}_{Y_i}, \pmb{PSR}_{Y_i}) &= \sum_{i=1}^N PSR_{Y_i} = \sum_{i=1}^N \left[  \hat{F}_Y(y_i) + \hat{F}_Y(y_{i-1})\epsilon_i - \epsilon_i   \right]\\
    g_3(\pmb{PSR}_{X_i}, \pmb{PSR}_{Y_i}) &= \sum_{i=1}^N PSR_{X_i}PSR_{Y_i} = \sum_{i=1}^N \left[  \hat{F}_X(x_i) + \hat{F}_X(x_{i-1})\delta_i - \delta_i   \right]  \cdot  \left[  \hat{F}_Y(y_i) + \hat{F}_Y(y_{i-1})\epsilon_i - \epsilon_i   \right]\\
    g_4(\pmb{PSR}_{X_i}, \pmb{PSR}_{Y_i}) &= \sum_{i=1}^N PSR_{X_i}^2 = \sum_{i=1}^N \left[  \hat{F}_X(x_i) + \hat{F}_X(x_{i-1})\delta_i - \delta_i   \right]^2\\
    g_5(\pmb{PSR}_{X_i}, \pmb{PSR}_{Y_i}) &= \sum_{i=1}^N PSR_{Y_i}^2 = \sum_{i=1}^N \left[  \hat{F}_Y(y_i) + \hat{F}_Y(y_{i-1})\epsilon_i - \epsilon_i   \right]^2\\
  \end{aligned}
$$
}}
...
{\footnotesize{  
  $$
  \begin{aligned}
    \left. \left(    \frac{\partial g_1(., .)}{\partial F_{X,1}}, ..., \frac{\partial g_1(., .)}{\partial F_{X,N}}, ~ \frac{\partial g_1(., .)}{\partial F_{Y,1}},  ..., \frac{\partial g_1(., .)}{\partial F_{Y,N}}   \right)    \right|_{\hat{F}_X, \hat{F}_Y} &= \left(   \delta_1, ~(1+\delta_2), ~(1+\delta_3), ...  , ~(1+\delta_{N-1}),  ~1,  \underbrace{~~0,~0,~0,...,~0,~0}_{N~for~Y} \right)\\
    %%%%%%
    \left. \left(    \frac{\partial g_2(., .)}{\partial F_{X,1}}, ..., \frac{\partial g_2(., .)}{\partial F_{X,N}}, ~ \frac{\partial g_2(., .)}{\partial F_{Y,1}},  ..., \frac{\partial g_2(., .)}{\partial F_{Y,N}}    \right)    \right|_{\hat{F}_X, \hat{F}_Y} &= \left( \underbrace{0,~0,~0,...,~0,~0}_{N~for~X},~~  \epsilon_1, ~(1+\epsilon_2), ~(1+\epsilon_3), ...  , ~(1+\epsilon_{N-1}),  ~1 \right)\\
    %%%%%%
    \left. \left(    \frac{\partial g_3(., .)}{\partial F_{X,1}}, ..., \frac{\partial g_3(., .)}{\partial F_{X,N}}, ~ \frac{\partial g_3(., .)}{\partial F_{Y,1}},  ..., \frac{\partial g_3(., .)}{\partial F_{Y,N}}   \right)    \right|_{\hat{F}_X, \hat{F}_Y} &= \left( \underbrace{R_{Y,1} +  R_{Y,2}\delta_2,  ...  , ~R_{Y,N-1} +  R_{Y,N}\delta_N,  ~R_{Y,N}}_{for~X},~~ \underbrace{R_{X,1} +  R_{X,2}\epsilon_2,  ...  , ~R_{X,N}}_{similar for Y} \right)\\
    %%%%%%
    \left. \left(    \frac{\partial g_4(., .)}{\partial F_{X,1}}, ..., \frac{\partial g_4(., .)}{\partial F_{X,N}}, ~ \frac{\partial g_4(., .)}{\partial F_{Y,1}},  ..., \frac{\partial g_4(., .)}{\partial F_{Y,N}}   \right)    \right|_{\hat{F}_X, \hat{F}_Y} &= 2\left( R_{X,1} +  R_{X,2}\delta_2, ~R_{X,2} +  R_{X,3}\delta_3,  ...  , ~R_{X,N-1} +  R_{X,N}\delta_N,  ~R_{X,N},  \underbrace{~~0,~0,~0,...,~0,~0}_{N~for~Y} \right)\\
    %%%%%%
    \left. \left(    \frac{\partial g_5(., .)}{\partial F_{X,1}}, ..., \frac{\partial g_5(., .)}{\partial F_{X,N}}, ~ \frac{\partial g_5(., .)}{\partial F_{Y,1}},  ..., \frac{\partial g_5(., .)}{\partial F_{Y,N}}   \right)    \right|_{\hat{F}_X, \hat{F}_Y} &= 2\left( \underbrace{~~0,~0,~0,...,~0,~0}_{N~for~X},~~ R_{Y,1} +  R_{Y,2}\epsilon_2, ~R_{Y,2} +  R_{Y,3}\epsilon_3,  ...  , ~R_{Y,N-1} +  R_{Y,N}\epsilon_N,  ~R_{Y,N} \right)\\
  \end{aligned}
  $$
}}
