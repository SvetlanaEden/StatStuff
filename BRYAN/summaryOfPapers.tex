...
First, let's introduce some notations. Times to event for sons and fathers are $S$ and $T$ respectively with the corresponding distribution functions of $F(S)$ and $G(T)$. We fix two points $S_0$ and $T_0$ and find the covariance of PRS under the assumption of perfect dependence expressed in the functional form $S\equiv T$. For brevity, we denote $F(S)=U$, $G(T)=V$, $F(S_0)=U_0$, and $G(T_0)=V_0$.\\
~\\
Before we continue, let's make sure we understand the probability distribution function of $(S, T)$. From \cite{nelsen2007introduction} we know that the joint \emph{cdf} of these two variables is $min(F(S), G(T))$. This does not help to derive the joint \emph{pdf} though (at least it did not help me). Let's denote the joint \emph{pdf} as $h(u, v)$. In order to figure our joint \emph{pdf}, let's use conditionality principle, $h(u, v)=h_{U|V=v}(u|v)\cdot h_V(v)$. We know that $F(S)=U \sim Unif(0, 1)$. This means that the marginal \emph{pdf} of $U$ is uniform, $h_V(v)=I_{[0\leq U \leq 1]}$. The conditional \emph{pdf}, $h_{U|V=v}$, equals $1$ when $U=v$ and $0$ when $U\neq v$, or, $h_{U|V=v} = C_{norm}I_{[0\leq V \leq 1]}\cdot I_{[U=v]}$, where $C_{norm}$ is a normalizing constant. This is a strange function that can be visualized as a piece of a plain, perpendicular to $(U,V)$ plain. It stretches from $(U,V) = (0, 0)$ to $(U,V) = (1,1)$. We will call this piece of plain a \emph{segment mass}. It has a height of 1 and a length of $\sqrt{2}$, therefore, in order for it to sum up to 1, it has to be divided by $\sqrt{2}$ or $C_{norm}=\sqrt{2}$.\\
~\\
...
Summing up all four integrals and taking into account the symmetry, we have an expression for \emph{Spearman} correlation of two identical variables, $S\equiv T$ conditional on the censoring point $(u_0, v_0)$:
	$$
	\begin{aligned}
    \rho_{S\equiv T}(u_0, v_0) &= \frac{1}{6}\left\{  (2u_0 - 1)^3 +1\right\} +  \frac{u_0}{4}\left\{  (2v_0 - 1)^2 -(2u_0 - 1)^2 \right\} + u_0 v_0 (1 - v_0), ~~~~when~u_0 < v_0\\
...
The derivations are very similar to the previous section with minor differences. When $T\equiv 1-S$, we have a \emph{segment-mass} again, but this time it stretches from $(U,V) = (1,0)$ to $(U,V) = (0,1)$. And again, in order for it to sum up to 1, it has to be divided by $\sqrt{2}$.\\
~\\
...
Integral $B$ is the following: 
	...
	\end{aligned}
	$$
Integral $C$ is very similar and keeping in mind that it should be symmetric:
	$$
	\begin{aligned}
...
We did not consider a case when $u_0+v0 = 1$ because it is very similar to the previous cases, except that both $A=0$ and $D=0$.\\
Summing up all four integrals, we have an expression for \emph{Spearman} for case $S\equiv 1-T$ conditional on the censoring point $(u_0, v_0)$:
	$$
	\begin{aligned}
    \rho_{S\equiv T}(u_0, v_0) &= -u_0v_0, ~~~~when~u_0 + v_0 < 1\\
    &= -\frac{1}{6}\left\{  (2u_0 - 1)^3 + (2v_0 - 1)^3 \right\} + u_0^2(u_0 - 1) + v_0^2(v_0 - 1), ~~~~when~u_0 \geq v_0
	\end{aligned}
	$$

\subsubsection{Perfect correlation under censoring, general case}
In case of perfect correlation, the expression for conditional (on censoring times) \emph{Spearman} correlation remains is the same for any time of functional dependency as long as the dependency is strictly increasing or decreasing. Suppose we have two variables $S$ and $T$ such that, $T = g(S)$, so $S = g^{-1}(T)$. Given that the distribution of $S$ is $F_S(s)$, let's find the distribution of $T$:
	$$
	\begin{aligned}
