...
The interest in survival outcome can be traced year 1846, a Hungarian doctor named Ignaz Semmelweis. He was a doctor at Vienna General Hospital maternity clinic, and collected data on the rate of death from childbed fever. He hypothesized that a child birth is helped by a doctor or a medical student, the women's rate of death from childbed fever is higher because of a disease that is caused by cadaverous particles that medical students and doctors have on on their hands after disecting cadavras. Jumping forward about 100 years, survival data was routinely collected and used by hospital, agencies, and governments [READ YOUR epidemiology book]. The recorded data were used to estimate rates of death, and the 
Kaplan-Meier [1958, p.459] in their famos paper, generalized the concept of censoring was introduced by Hald in 1949 ["Maximum Likelihood estimation of th eparameters of a normal distribution which is trancated at a known point", \emph{Skandinavisk Aktuarielidskrift, 32 (1949)}, 119-134]. Cox (1972) extended Kaplan\&Meier results and introduced a regression model for censored suvival data. His model utilized a concept of a \emph{hazard rate}:
$$
\begin{aligned}
...
\end{aligned}
$$
Where $\mathcal{S}(s|y) = 1-F(s|y)$, and $F(s|y)$ and $f(s|y)$ are cumulative distribution function and density function respectively.\\
Basu (1971) [get this paper from the library ??? this does not add up because Cox wrote his paper later] defined a bivariate survival rate as:
$$
\begin{aligned}
...
\end{aligned}
$$
Firther, Clayton (1978), in order to set a framework for bivariate survival association between fathers and sons, defined a measure similar to harzard ratio. He wanted this measure to be easy to compute, to be expressible as a constant ratio of age-specific rates,  and to be symmetrical in two variables. If we denote time to event for fathers to be $s$ and time to event for sons to be $t$, this measure can be written in the following way:
$$
\begin{aligned}
	\theta = \frac{\lambda_s(s_0|t=t_0)}{\lambda_s(s_0|t>t_0)} = \frac{\lambda_t(t_0|s=s_0)}{\lambda_t(t_0|s>s_0)}
\end{aligned}
$$
The requirement of symmetry is motivated by the fact than neither son nor father are causes of event, rather, there is an unobservable variable that is associated with the event for both of them. This measure can be interpreted for sons (as much as for fathers) in the following way: how much more likely for a son to have an event at time $s_0$ given that his fathers had an event at time $t_0$ compared to a son whose father survived untile time $t_0$ without the event. Independence between fathers and sons would mean $\theta = 1$, positive association $\theta > 1$ and negative association $\theta < 1$.\\
\textbf{To-do list}
\begin{enumerate}
  \item Find in Cox paper: he mentioned bivariate survial of different kind from Clayton's: two events within one individual.
  \item [Elaborate on the fact that he introduce a whole model (not just a coefficient), showed how to estimate it and added sections on discrete time, and covariates].
\end{enumerate}
