...
After writing the simulation plan, I focused on the problem of generating confidence intervals for conditional correlation as a function of calendar time. The problem is that for each event, the cox model is very significant, but the A matrix (the matrix required for the M-estimation) is computationally singular. I tried several things.
1) Changed calendar time to start from 0. This worked for children. For adults, this almost worked. For adults, I am using M-estimation with full likelihood for one variable and M-estiamtion that assumes known hazard for the other.
2) I do not know why this is the case, but am investigating this.
...
I was also working on incorporating the bias into the code. What was surprising was that the unadjusted method, KM, worked horribly (p.23). Well, I guess this should make sense. I'd appreciate the feedback for this table.
I am working on some problem with simulations (it fails on ACCRE, but not on my computer.)
Worked on nice plots of conditional correlation as a function of age and cd4. Compared different confidence intervals (full likelyhood and KM). KM intervals were wider.
...
Explored the issue with conditional correlation of the calendar time. The problem was with the variable canlendar time. For some reason, when it is in years, the baseline hazard becomes enormous. Then the M-estimation gets all messed up. When I switched to time from the first year, it worked for children. For adults it was still a problem, so for one of the variables I used "known hazard" approach in M-estimation.
