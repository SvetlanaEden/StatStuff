...
\subsection{Ding, Wang, 2012: Testing Independence for Bivariate Current Status Data}
Suggested two statistics for testing independence for bivariate \emph{current status data}. One is based on counts (how many events, non-events, and so forth). The other one is:
	$$
	\begin{aligned}
		 E[cov(\delta_1, \delta_2)|C_1,C_2] = E\left\{ [\delta_1 - F_1(C_1)][\delta_1 - F_1(C_1)]  \right\}\\
	\end{aligned}
	$$
Which is \textbf{exactly} covaraince of \textbf{PSR} for \emph{current status data}.
